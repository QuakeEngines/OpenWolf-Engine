...
long S_OGG_Callback_tell( void* datasource )
{
    ...
    return ( S64 )( ( ( snd_stream_t* ) datasource ) -> pos );
}
...
snd_stream_t* S_OGG_CodecOpenStream( StringEntry filename )
{
    ...
    vf = static_cast<OggVorbis_File*>( memorySystem->Malloc( sizeof( OggVorbis_File ) ) );
    if ( !vf )
    {
    ...
    numSamples = ( S64 )ov_pcm_total( vf, 0 );
...
#	endif // Q3_BIG_ENDIAN
    ...
    bytesRead = 0;
    bytesLeft = bytes;
    bufPtr = static_cast<UTF8*>( buffer );
