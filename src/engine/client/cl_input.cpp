...
void CL_KeyMove( usercmd_t* cmd )
{
    ...
    if ( kb[KB_STRAFE].active )
    {
        side += ( S32 )( movespeed * CL_KeyState( &kb[KB_RIGHT] ) );
        side -= ( S32 )( movespeed * CL_KeyState( &kb[KB_LEFT] ) );
    }
    
    side += ( S32 )( movespeed * CL_KeyState( &kb[KB_MOVERIGHT] ) );
    side -= ( S32 )( movespeed * CL_KeyState( &kb[KB_MOVELEFT] ) );
    ...
    up += ( S32 )( movespeed * CL_KeyState( &kb[KB_UP] ) );
    up -= ( S32 )( movespeed * CL_KeyState( &kb[KB_DOWN] ) );
    
    forward += ( S32 )( movespeed * CL_KeyState( &kb[KB_FORWARD] ) );
    forward -= ( S32 )( movespeed * CL_KeyState( &kb[KB_BACK] ) );
...
void CL_MouseMove( usercmd_t* cmd )
{
    ...
    {
        mx = ( F32 )cl.mouseDx[cl.mouseIndex];
        my = ( F32 )cl.mouseDy[cl.mouseIndex];
    }
    ...
    if ( cl_mouseAccel->value != 0.0f )
    {
        if ( cl_mouseAccel->value != 0.0f && cl_mouseAccelOffset->value > 0.0f )
        {
            ...
            rate = sqrtf( mx * mx + my * my ) / ( F32 ) frame_msec;
    ...
    if ( kb[KB_STRAFE].active )
        cmd->rightmove = ClampChar( cmd->rightmove + ( S32 )( m_side->value * mx ) );
    else
        cl.viewangles[YAW] -= ( S32 )( m_yaw->value * mx );
        
    if ( ( kb[KB_MLOOK].active || cl_freelook->integer ) && !kb[KB_STRAFE].active )
        cl.viewangles[PITCH] += ( S32 )( m_pitch->value * my );
    else
        cmd->forwardmove = ClampChar( cmd->forwardmove - ( S32 )( m_forward->value * my ) );
}
...
void CL_WritePacket( void )
{
    ...
    idClientNetworkChainSystemLocal::Netchan_Transmit( &clc.netchan, &buf );
    ...
    while ( clc.netchan.unsentFragments )
    {
        ...
        idClientNetworkChainSystemLocal::Netchan_TransmitNextFragment( &clc.netchan );
    }
}
