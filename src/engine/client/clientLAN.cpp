...
S32 idClientLANSystemLocal::AddServer( S32 source, StringEntry name, StringEntry address )
{
    ...
    {
        networkChainSystem->StringToAdr( address, &adr, NA_IP );
...
void idClientLANSystemLocal::RemoveServer( S32 source, StringEntry addr )
{
    ...
    if ( servers )
    {
        ...
        networkChainSystem->StringToAdr( addr, &comp, NA_IP );
...
void idClientLANSystemLocal::GetServerAddressString( S32 source, S32 n, UTF8* buf, S32 buflen )
{
    switch ( source )
    {
        case AS_LOCAL:
            ...
            {
                Q_strncpyz( buf, networkSystem->AdrToString( cls.localServers[n].adr ), buflen );
                return;
            }
            break;
        ...
        case AS_GLOBAL:
            ...
            {
                Q_strncpyz( buf, networkSystem->AdrToString( cls.globalServers[n].adr ), buflen );
                return;
            }
            break;
        ...
        case AS_FAVORITES:
            if ( n >= 0 && n < MAX_OTHER_SERVERS )
            {
                Q_strncpyz( buf, networkSystem->AdrToString( cls.favoriteServers[n].adr ), buflen );
                return;
            }
            break;
    }
...
void idClientLANSystemLocal::GetServerInfo( S32 source, S32 n, UTF8* buf, S32 buflen )
{
    ...
    if ( server && buf )
    {
        ...
        Info_SetValueForKey( info, "addr", networkSystem->AdrToString( server->adr ) );
