...
void CMod_LoadShaders( lump_t* l )
{
    ...
    cm.shaders = ( dshader_t* )memorySystem->Alloc( count * sizeof( *cm.shaders ), h_high );
    cm.numShaders = count;
...
void CMod_LoadSubmodels( lump_t* l )
{
    ...
    cm.cmodels = ( cmodel_t* )memorySystem->Alloc( count * sizeof( *cm.cmodels ), h_high );
    ...
    for ( i = 0; i < count; i++, in++, out++ )
    {
        ...
        indexes = ( S32* )memorySystem->Alloc( out->leaf.numLeafBrushes * 4, h_high );
        ...
        indexes = ( S32* )memorySystem->Alloc( out->leaf.numLeafSurfaces * 4, h_high );
...
void CMod_LoadNodes( lump_t* l )
{
    ...
    cm.nodes = ( cNode_t* )memorySystem->Alloc( count * sizeof( *cm.nodes ), h_high );
    cm.numNodes = count;
...
void CMod_LoadBrushes( lump_t* l )
{
    ...
    cm.brushes = ( cbrush_t* )memorySystem->Alloc( ( BOX_BRUSHES + count ) * sizeof( *cm.brushes ), h_high );
    cm.numBrushes = count;
...
void CMod_LoadLeafs( lump_t* l )
{
    ...
    cm.leafs = ( cLeaf_t* )memorySystem->Alloc( ( BOX_LEAFS + count ) * sizeof( *cm.leafs ), h_high );
    cm.numLeafs = count;
    ...
    cm.areas = ( cArea_t* )memorySystem->Alloc( cm.numAreas * sizeof( *cm.areas ), h_high );
    cm.areaPortals = ( S32* )memorySystem->Alloc( cm.numAreas * cm.numAreas * sizeof( *cm.areaPortals ), h_high );
}
...
void CMod_LoadPlanes( lump_t* l )
{
    ...
    cm.planes = ( cplane_t* )memorySystem->Alloc( ( BOX_PLANES + count ) * sizeof( *cm.planes ), h_high );
    cm.numPlanes = count;
...
void CMod_LoadLeafBrushes( lump_t* l )
{
    ...
    cm.leafbrushes = ( S32* )memorySystem->Alloc( ( BOX_LEAF_BRUSHES + count ) * sizeof( *cm.leafbrushes ), h_high );
...
void CMod_LoadLeafSurfaces( lump_t* l )
{
    ...
    cm.leafsurfaces = ( S32* )memorySystem->Alloc( count * sizeof( *cm.leafsurfaces ), h_high );
...
void CMod_LoadBrushSides( lump_t* l )
{
    ...
    cm.brushsides = ( cbrushside_t* )memorySystem->Alloc( ( BOX_SIDES + count ) * sizeof( *cm.brushsides ), h_high );
...
static void CMod_CreateBrushSideWindings( void )
{
    ...
    for ( i = 0; i < cm.numBrushes; i++ )
    {
        ...
        tempEdges = ( cbrushedge_t* )memorySystem->Malloc( sizeof( cbrushedge_t ) * numEdges );
        brush->numEdges = 0;
...
void CMod_LoadEntityString( lump_t* l )
{
    ...
    cm.entityString = ( UTF8* )memorySystem->Alloc( l->filelen, h_high );
...
void CMod_LoadVisibility( lump_t* l )
{
    ...
    if ( !len )
    {
        ...
        cm.visibility = ( U8* )memorySystem->Alloc( cm.clusterBytes, h_high );
    ...
    cm.vised = true;
    cm.visibility = ( U8* )memorySystem->Alloc( len, h_high );
...
void CMod_LoadSurfaces( lump_t* surfs, lump_t* verts, lump_t* indexesLump )
{
    ...
    cm.surfaces = ( cSurface_t** )memorySystem->Alloc( cm.numSurfaces * sizeof( cm.surfaces[0] ), h_high );
    ...
    for ( i = 0; i < count; i++, in++ )
    {
        if ( LittleLong( in->surfaceType ) == MST_PATCH )
        {
            ...
            cm.surfaces[i] = surface = ( cSurface_t* )memorySystem->Alloc( sizeof( *surface ), h_high );
...
void CM_InitBoxHull( void )
{
    ...
    box_brush->edges = ( cbrushedge_t* )memorySystem->Alloc( sizeof( cbrushedge_t ) * 12, h_low );
