...
static S32 CM_PlaneEqual( cPlane_t* p, F32 plane[4], S32* flipped )
{
    ...
    if ( Q_fabs( p->plane[0] - invplane[0] ) < NORMAL_EPSILON && Q_fabs( p->plane[1] - invplane[1] ) < NORMAL_EPSILON
            && Q_fabs( p->plane[2] - invplane[2] ) < NORMAL_EPSILON && Q_fabs( p->plane[3] - invplane[3] ) < DIST_EPSILON )
    {
        *flipped = true;
        return true;
    }
...
static void CM_SurfaceCollideFromGrid( cGrid_t* grid, cSurfaceCollide_t* sc )
{
    ...
    sc->facets = ( cFacet_t* )memorySystem->Alloc( numFacets * sizeof( *sc->facets ), h_high );
    ::memcpy( sc->facets, facets, numFacets * sizeof( *sc->facets ) );
    sc->planes = ( cPlane_t* )memorySystem->Alloc( numPlanes * sizeof( *sc->planes ), h_high );
