...
void CM_TracePointThroughSurfaceCollide( traceWork_t* tw, const cSurfaceCollide_t* sc )
{
    ...
    for ( i = 0, facet = sc->facets; i < sc->numFacets; i++, facet++ )
    {
        ...
        if ( j == facet->numBorders )
        {
            ...
            d1 = DotProduct( tw->start, planes->plane ) - planes->plane[3] + offset;
            d2 = DotProduct( tw->end, planes->plane ) - planes->plane[3] + offset;
            tw->trace.fraction = ( F32 )( ( d1 - SURFACE_CLIP_EPSILON ) / ( d1 - d2 ) );
...
S32 CM_CheckFacetPlane( F32* plane, vec3_t start, vec3_t end, F32* enterFrac, F32* leaveFrac, S32* hit )
{
    ...
    *hit = false;
    ...
    {
        // enter
        f = ( F32 )( ( d1 - SURFACE_CLIP_EPSILON ) / ( d1 - d2 ) );
        if ( f < 0 )
        {
            f = 0;
        }
...
void CM_TraceThroughBrush( traceWork_t* tw, cbrush_t* brush )
{
    ...
    if ( enterFrac < leaveFrac )
    {
        if ( enterFrac > -1 && enterFrac < tw->trace.fraction )
        {
            ...
            tw->trace.fraction = ( F32 )enterFrac;
            if ( clipplane != NULL )
            {
...
==================
*/
void idCollisionModelManagerLocal::TransformedBoxTrace( trace_t* results, const vec3_t start, const vec3_t end, const vec3_t mins,
        const vec3_t maxs, clipHandle_t model, S32 brushmask, const vec3_t origin, const vec3_t angles, traceType_t type )
{
    trace_t         trace;
...
void idCollisionModelManagerLocal::TransformedBiSphereTrace( trace_t* results, const vec3_t start, const vec3_t end, F32 startRad, F32 endRad, clipHandle_t model, S32 mask, const vec3_t origin )
{
    ...
    collisionModelManagerLocal.BiSphereTrace( &trace, start_l, end_l, startRad, endRad, model, mask );
