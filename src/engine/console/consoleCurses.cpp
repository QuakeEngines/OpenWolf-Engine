...
void idConsoleCursesLocal::UpdateClock( void )
{
    ...
    ColorPrint( clockwin, va( "^0[^3%02d%c%02d^0] ", realtime.tm_hour, ( realtime.tm_sec & 1 ) ? ':' : ' ', realtime.tm_min ), true );
    ...
    while ( 1 )
    {
        ...
        switch ( chr )
        {
            ...
            case KEY_ENTER:
                ...
                consoleHistorySystem->HistoryAdd( text );
            ...
            case KEY_STAB:
                ...
                input_field.cursor = ( S32 )::strlen( input_field.buffer );
                continue;
            case '\f':
                Resize();
                continue;
            case KEY_LEFT:
                ...
                continue;
            case KEY_RIGHT:
                if ( input_field.cursor < ( S32 )::strlen( input_field.buffer ) )
                {
                    input_field.cursor++;
                }
                continue;
            case KEY_UP:
            {
                StringEntry field = consoleHistorySystem->HistoryPrev();
                if ( field )
                {
            ...
            case KEY_DOWN:
            {
                StringEntry field = consoleHistorySystem->HistoryNext();
                if ( field )
                {
                    strcpy( input_field.buffer, field );
                }
