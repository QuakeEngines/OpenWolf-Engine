...
public:
    ...
    static S64 HashFileName( StringEntry fname, size_t hashSize );
    static fileHandle_t HandleForFile( void );
    static S32 filelength( fileHandle_t f );
    static void ReplaceSeparators( UTF8* path );
    static void BuildOSHomePath( UTF8* ospath, S32 size, S32 qpath );
    static bool Remove( StringEntry osPath );
    static bool SV_FileExists( StringEntry file );
    static fileHandle_t FOpenFileUpdate( StringEntry filename, S32* length );
    static S32 FOpenFileDirect( StringEntry filename, fileHandle_t* f );
    static bool FilenameCompare( StringEntry s1, StringEntry s2 );
    static UTF8* ShiftedStrStr( StringEntry string, StringEntry substring, S32 shift );
    static bool AllowDeletion( UTF8* filename );
    static S32 DeleteDir( UTF8* dirname, bool nonEmpty, bool recursive );
    static S32 OSStatFile( UTF8* ospath );
    static S32 FPrintf( fileHandle_t f, StringEntry fmt, ... );
    static pack_t* LoadZipFile( StringEntry zipfile, StringEntry basename );
    static S32 ReturnPath( StringEntry zname, UTF8* zpath, S32* depth );
    static S32 AddFileToList( UTF8* name, UTF8* list[MAX_FOUND_FILES], S32 nfiles );
    static UTF8** ListFilteredFiles( StringEntry path, StringEntry extension, UTF8* filter, S32* numfiles );
    static U32 CountFileList( UTF8** list );
    static UTF8** ConcatenateFileLists( UTF8** list0, UTF8** list1, UTF8** list2 );
    static S32 GetModList( UTF8* listbuf, S32 bufsize );
    static void Dir_f( void );
    static void ConvertPath( UTF8* s );
    static S32 PathCmp( StringEntry s1, StringEntry s2 );
    static void SortFileList( UTF8** filelist, S32 numfiles );
    ...
    static bool IsExt( StringEntry filename, StringEntry ext, S32 namelen );
    static void AddGameDirectory( StringEntry path, StringEntry dir );
    static void ReorderPurePaks( void );
    static void Startup( StringEntry gameName );
    static bool FileInPathExists( StringEntry testpath );
};
