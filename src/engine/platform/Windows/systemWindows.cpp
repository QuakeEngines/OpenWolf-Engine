...
UTF8* idSystemLocal::SysGetClipboardData( void )
{
    ...
    if ( OpenClipboard( nullptr ) != 0 )
    {
        ...
        if ( ( hClipboardData = GetClipboardData( CF_TEXT ) ) != 0 )
        {
            ...
            {
                data = ( UTF8* )memorySystem->Malloc( GlobalSize( hClipboardData ) + 1 );
                Q_strncpyz( data, cliptext, ( S32 )GlobalSize( hClipboardData ) );
...
bool idSystemLocal::strgtr( StringEntry s0, StringEntry s1 )
{
    ...
    l0 = ( S32 )std::strlen( s0 );
    l1 = ( S32 )std::strlen( s1 );
...
==============
*/
void idSystemLocal::SysSleep( S32 msec )
{
    if ( msec == 0 )
    {
        return;
...
{
    return ShellExecute( nullptr, nullptr, url, nullptr, nullptr, SW_SHOWNORMAL ) ? true : false ;
}
...
void idSystemLocal::ErrorDialog( StringEntry error )
{
    ...
    f = _open( ospath, O_CREAT | O_TRUNC | O_WRONLY, 0640 );
    if ( f == -1 )
    {
...
void idSystemLocal::StartProcess( UTF8* exeName, bool doexit )
{
    ...
    if ( !CreateProcess( nullptr, exeName, nullptr, nullptr, 0, 0, nullptr, nullptr, &si, &pi ) )
    {
        if ( doexit )
        {
