...
static convar_t* in_raw = nullptr;
...
===============
*/
void idSystemLocal::PrintKey( const SDL_Keysym* keysym, keyNum_t key, bool down )
{
    if ( down )
    {
        Com_Printf( "+ " );
    ...
    Com_Printf( " Q:0x%02x(%s)\n", key, clientKeysSystem->Key_KeynumToString( key ) );
}
...
bool idSystemLocal::IsConsoleKey( keyNum_t key, S32 character )
{
    ...
    if ( cl_consoleKeys->modified )
    {
        ...
        while ( numConsoleKeys < MAX_CONSOLE_KEYS )
        {
            ...
            {
                c->type = QUAKE_KEY;
                c->u.key = ( keyNum_t )clientKeysSystem->Key_StringToKeynum( token );
...
===============
*/
keyNum_t idSystemLocal::TranslateSDLToQ3Key( SDL_Keysym* keysym, bool down )
{
            ...
            if ( ( ( U8* )&hats )[i] != ( ( U8* )&stick_state.oldhats )[i] )
            {
                ...
                switch ( ( ( U8* )&stick_state.oldhats )[i] )
                {
                    case SDL_HAT_UP:
                        ...
                        break;
                    case SDL_HAT_RIGHT:
                        Com_QueueEvent( eventTime, SYSE_KEY, hat_keys[4 * i + 1], false, 0, nullptr );
                        break;
                    case SDL_HAT_DOWN:
                        Com_QueueEvent( eventTime, SYSE_KEY, hat_keys[4 * i + 2], false, 0, nullptr );
                        break;
                    case SDL_HAT_LEFT:
                        Com_QueueEvent( eventTime, SYSE_KEY, hat_keys[4 * i + 3], false, 0, nullptr );
                        break;
...
void idSystemLocal::ProcessEvents( S32 eventTime )
{
    ...
    while ( SDL_PollEvent( &_event ) )
    {
        ...
        {
            case SDL_KEYDOWN:
                if ( _event.key.repeat && clientGUISystem->GetCatcher() == 0 )
                {
                    break;
                }
                
                if ( ( key = TranslateSDLToQ3Key( &_event.key.keysym, true ) ) )
                {
            ...
            case SDL_KEYUP:
                if ( ( key = TranslateSDLToQ3Key( &_event.key.keysym, false ) ) )
                {
            ...
            case SDL_MOUSEMOTION:
                if ( mouseActive )
                {
                    mx += _event.motion.xrel;
                    my += _event.motion.yrel;
                }
                break;
            ...
            case SDL_MOUSEBUTTONUP:
            {
                ...
                switch ( _event.button.button )
                {
                    ...
                    case SDL_BUTTON_X2:
                        ...
                        break;
                    default:
                        b = K_AUX1 + ( _event.button.button - SDL_BUTTON_X2 + 1 ) % 16;
                        break;
                }
                Com_QueueEvent( eventTime, SYSE_KEY, b, ( _event.type == SDL_MOUSEBUTTONDOWN ? true : false ), 0, nullptr );
            }
            break;
            ...
            case SDL_QUIT:
                cmdBufferSystem->ExecuteText( EXEC_NOW, "quit\n" );
                break;
            ...
            case SDL_WINDOWEVENT:
                switch ( _event.window.event )
                {
                    case SDL_WINDOWEVENT_RESIZED:
                    {
                        ...
                        cvarSystem->SetValue( "r_customwidth", ( F32 )width );
                        cvarSystem->SetValue( "r_customheight", ( F32 )height );
...
void idSystemLocal::Frame( void )
{
    ...
    loading = ( bool )( cls.state != CA_DISCONNECTED && cls.state != CA_ACTIVE && !( clientGUISystem->GetCatcher() & KEYCATCH_UI ) );
    
    // update isFullscreen since it might of changed since the last vid_restart
    cls.glconfig.isFullscreen = cvarSystem->VariableIntegerValue( "r_fullscreen" ) != 0;
    
    if ( !cls.glconfig.isFullscreen && ( clientGUISystem->GetCatcher() & KEYCATCH_CONSOLE ) )
    {
        // Console is down in windowed mode
        idsystem->DeactivateMouse();
    }
...
===============
*/
void idSystemLocal::InputInit( void* windowData )
{
    S32 appState;
    
    if ( !SDL_WasInit( SDL_INIT_VIDEO ) )
    {
        Com_Error( ERR_FATAL, "idSystemLocal::Init called before SDL_Init( SDL_INIT_VIDEO )" );
        return;
    }
    ...
    in_raw = cvarSystem->Get( "in_raw", "1", CVAR_ARCHIVE, "Raw mouse input" );
    ...
    cvarSystem->SetValue( "com_unfocused", !( appState & SDL_WINDOW_INPUT_FOCUS ) );
    cvarSystem->SetValue( "com_minimized", ( F32 )( appState & SDL_WINDOW_MINIMIZED ) );
...
{
    ShutdownJoystick();
    systemLocal.InputInit( SDL_window );
}
