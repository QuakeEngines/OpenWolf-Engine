...
idRenderSystelModelIQMLocal renderSystemIQMModelLocal;

/*
===============
idRenderSystelModelIQMLocal::idRenderSystelModelIQMLocal
===============
*/
idRenderSystelModelIQMLocal::idRenderSystelModelIQMLocal( void )
{
}
...
/*
===============
idRenderSystelModelIQMLocal::CheckRange
===============
*/
bool idRenderSystelModelIQMLocal::CheckRange( iqmHeader_t* header, U32 offset, S32 count, S32 size )
{
...
/*
===============
idRenderSystelModelIQMLocal::Matrix34Multiply

"multiply" 3x4 matrices, these are assumed to be the top 3 rows
of a 4x4 matrix with the last row = (0 0 0 1)
===============
*/
void idRenderSystelModelIQMLocal::Matrix34Multiply( const F32* a, const F32* b, F32* out )
{
...
/*
===============
idRenderSystelModelIQMLocal::JointToMatrix
===============
*/
void idRenderSystelModelIQMLocal::JointToMatrix( const quat_t rot, const vec3_t scale, const vec3_t trans, F32* mat )
{
...
/*
===============
idRenderSystelModelIQMLocal::Matrix34Invert
===============
*/
void idRenderSystelModelIQMLocal::Matrix34Invert( const F32* inMat, F32* outMat )
{
    F32 invSqrLen, *v;
    vec3_t trans;
...
/*
===============
idRenderSystelModelIQMLocal::QuatSlerp
===============
*/
void idRenderSystelModelIQMLocal::QuatSlerp( const quat_t from, const quat_t _to, F32 fraction, quat_t out )
{
...
/*
===============
idRenderSystelModelIQMLocal::QuatNormalize2
===============
*/
F32 idRenderSystelModelIQMLocal::QuatNormalize2( const quat_t v, quat_t out )
{
    F32	length, ilength;
...
/*
=================
idRenderSystelModelIQMLocal::LoadIQM

Load an IQM model and compute the joint matrices for every frame.
=================
*/
bool idRenderSystelModelIQMLocal::LoadIQM( model_t* mod, void* buffer, U32 filesize, StringEntry mod_name )
{
    iqmHeader_t* header;
...
#define	LL(x) x=LittleLong(x)
    ...
    {
        clientMainSystem->RefPrintf( PRINT_WARNING, "idRenderSystelModelIQMLocal::LoadIQM: %s is a unsupported IQM version (%d), only version %d is supported.\n",
    ...
    {
        clientMainSystem->RefPrintf( PRINT_WARNING, "idRenderSystelModelIQMLocal::LoadIQM: %s has more than %d joints (%d).\n",
    ...
    if ( header->num_meshes )
    {
        ...
        if ( CheckRange( header, header->ofs_vertexarrays, header->num_vertexarrays, sizeof( iqmVertexArray_t ) ) )
        {
            return false;
        }
        ...
        for ( i = 0; i < header->num_vertexarrays; i++, vertexarray++ )
        {
            ...
            switch ( vertexarray->format )
            {
                ...
                case IQM_UBYTE:
                    ...
                    if ( CheckRange( header, vertexarray->offset, n, sizeof( U8 ) ) )
                    {
                        return false;
                    }
                    break;
                ...
                case IQM_FLOAT:
                    // 4-U8 swap
                    if ( CheckRange( header, vertexarray->offset, n, sizeof( F32 ) ) )
                    {
                        return false;
                    }
                    
                    intPtr = ( U32* )( ( U8* )header + vertexarray->offset );
        ...
        {
            clientMainSystem->RefPrintf( PRINT_WARNING, "idRenderSystelModelIQMLocal::LoadIQM: %s is missing IQM_POSITION, IQM_NORMAL, and/or IQM_TEXCOORD array.\n", mod_name );
            return false;
        }
        ...
        if ( header->num_joints )
        {
            ...
            {
                clientMainSystem->RefPrintf( PRINT_WARNING, "idRenderSystelModelIQMLocal::LoadIQM: %s is missing IQM_BLENDINDEXES and/or IQM_BLENDWEIGHTS array.\n", mod_name );
                return false;
            }
        }
        else
        ...
        {
            clientMainSystem->RefPrintf( PRINT_WARNING, "idRenderSystelModelIQMLocal::LoadIQM: %s is missing IQM_TANGENT array.\n", mod_name );
            return false;
        }
        
        // check and swap triangles
        if ( CheckRange( header, header->ofs_triangles, header->num_triangles, sizeof( iqmTriangle_t ) ) )
        {
            return false;
        }
        ...
        if ( CheckRange( header, header->ofs_meshes, header->num_meshes, sizeof( iqmMesh_t ) ) )
        {
            return false;
        }
        ...
        for ( i = 0; i < header->num_meshes; i++, mesh++ )
        {
            ...
            {
                Q_strncpyz( meshName, reinterpret_cast<UTF8*>( header ) + header->ofs_text + mesh->name, sizeof( meshName ) );
            }
            else
            {
                meshName[0] = '\0';
            ...
            {
                clientMainSystem->RefPrintf( PRINT_WARNING, "idRenderSystelModelIQMLocal::LoadIQM: %s has more than %i verts on %s (%i).\n",
            ...
            {
                clientMainSystem->RefPrintf( PRINT_WARNING, "idRenderSystelModelIQMLocal::LoadIQM: %s has more than %i triangles on %s (%i).\n",
    ...
    {
        clientMainSystem->RefPrintf( PRINT_WARNING, "idRenderSystelModelIQMLocal::LoadIQM: %s has %d poses and %d joints, must have the same number or 0 poses\n",
    ...
    if ( header->num_joints )
    {
        ...
        if ( CheckRange( header, header->ofs_joints, header->num_joints, sizeof( iqmJoint_t ) ) )
        {
            return false;
        }
        ...
        for ( i = 0; i < header->num_joints; i++, joint++ )
        {
            ...
            if ( joint->parent < -1 || joint->parent >= ( S32 )header->num_joints || joint->name >= ( S32 )header->num_text )
            {
                return false;
            }
            
            joint_names += ::strlen( reinterpret_cast<UTF8*>( header ) + header->ofs_text + joint->name ) + 1;
        }
    }
    ...
    if ( header->num_poses )
    {
        ...
        if ( CheckRange( header, header->ofs_poses, header->num_poses, sizeof( iqmPose_t ) ) )
        {
            return false;
        }
    ...
    if ( header->ofs_bounds )
    {
        ...
        if ( CheckRange( header, header->ofs_bounds, header->num_frames, sizeof( *bounds ) ) )
        {
            return false;
        }
    ...
    if ( header->num_meshes )
    {
        ...
        // triangles
        size += header->num_triangles * 3 * sizeof( S32 );
        // positions
        size += header->num_vertexes * 3 * sizeof( F32 );
        // texcoords
        size += header->num_vertexes * 2 * sizeof( F32 );
        // normals
        size += header->num_vertexes * 3 * sizeof( F32 );
        ...
        {
            // tangents
            size += header->num_vertexes * 4 * sizeof( F32 );
        }
        
        if ( vertexArrayFormat[IQM_COLOR] != -1 )
        {
            // colors
            size += header->num_vertexes * 4 * sizeof( U8 );
        }
        
        if ( allocateInfluences )
        {
            // influences
            size += header->num_vertexes * sizeof( S32 );
            // influenceBlendIndexes
            size += allocateInfluences * 4 * sizeof( U8 );
            ...
            {
                // influenceBlendWeights
                size += allocateInfluences * 4 * sizeof( F32 );
            }
        }
    }
    ...
    if ( header->num_joints )
    {
        ...
        size += joint_names;
        // joint parents
        size += header->num_joints * sizeof( S32 );
        // bind joint matricies
        size += header->num_joints * 12 * sizeof( F32 );
        // inverse bind joint matricies
        size += header->num_joints * 12 * sizeof( F32 );
    }
    ...
    {
        // pose transforms
        size += header->num_poses * header->num_frames * sizeof( iqmTransform_t );
    }
    
    if ( header->ofs_bounds )
    {
        // model bounds
        size += header->num_frames * 6 * sizeof( F32 );
    }
    ...
    if ( header->num_meshes )
    {
        ...
        // triangles
        dataPtr += header->num_triangles * 3 * sizeof( S32 );
        
        iqmData->positions = ( F32* )dataPtr;
        // positions
        dataPtr += header->num_vertexes * 3 * sizeof( F32 );
        
        iqmData->texcoords = ( F32* )dataPtr;
        // texcoords
        dataPtr += header->num_vertexes * 2 * sizeof( F32 );
        ...
        if ( vertexArrayFormat[IQM_TANGENT] != -1 )
        {
            ...
            // tangents
            dataPtr += header->num_vertexes * 4 * sizeof( F32 );
        }
        ...
        if ( vertexArrayFormat[IQM_COLOR] != -1 )
        {
            ...
            // colors
            dataPtr += header->num_vertexes * 4 * sizeof( U8 );
        }
        ...
        if ( allocateInfluences )
        {
            ...
            // influences
            dataPtr += header->num_vertexes * sizeof( S32 );
            
            iqmData->influenceBlendIndexes = ( U8* )dataPtr;
            // influenceBlendIndexes
            dataPtr += allocateInfluences * 4 * sizeof( U8 );
            ...
            else if ( vertexArrayFormat[IQM_BLENDWEIGHTS] == IQM_FLOAT )
            {
                ...
                // influenceBlendWeights
                dataPtr += allocateInfluences * 4 * sizeof( F32 );
            }
        }
    }
    if ( header->num_joints )
    {
        iqmData->jointNames = reinterpret_cast<UTF8*>( dataPtr );
        // joint names
        dataPtr += joint_names;
        
        iqmData->jointParents = ( S32* )dataPtr;
        // joint parents
        dataPtr += header->num_joints * sizeof( S32 );
        
        iqmData->bindJoints = ( F32* )dataPtr;
        // bind joint matricies
        dataPtr += header->num_joints * 12 * sizeof( F32 );
    ...
    if ( header->num_poses )
    {
        ...
        // pose transforms
        dataPtr += header->num_poses * header->num_frames * sizeof( iqmTransform_t );
    }
    ...
    if ( header->ofs_bounds )
    {
        ...
        // model bounds
        dataPtr += header->num_frames * 6 * sizeof( F32 );
    }
    else if ( header->num_meshes && header->num_frames == 0 )
    {
        iqmData->bounds = ( F32* )dataPtr;
        // model bounds
        dataPtr += 6 * sizeof( F32 );
    }
    ...
    if ( header->num_meshes )
    {
        ...
        str = reinterpret_cast<UTF8*>( header ) + header->ofs_text;
        ...
        for ( i = 0; i < header->num_meshes; i++, mesh++, surface++ )
        {
            ...
            surface->shader = idRenderSystemShaderLocal::FindShader( str + mesh->material, LIGHTMAP_NONE, true );
        ...
        for ( i = 0; i < header->num_vertexarrays; i++, vertexarray++ )
        {
            ...
            {
                case IQM_POSITION:
                    ::memcpy( iqmData->positions, ( U8* )header + vertexarray->offset, n * sizeof( F32 ) );
                    break;
                case IQM_NORMAL:
                    ::memcpy( iqmData->normals, ( U8* )header + vertexarray->offset, n * sizeof( F32 ) );
                    break;
                case IQM_TANGENT:
                    ::memcpy( iqmData->tangents, ( U8* )header + vertexarray->offset, n * sizeof( F32 ) );
                    break;
                case IQM_TEXCOORD:
                    ::memcpy( iqmData->texcoords, ( U8* )header + vertexarray->offset, n * sizeof( F32 ) );
                    break;
                case IQM_BLENDINDEXES:
                case IQM_BLENDWEIGHTS:
                    break;
                case IQM_COLOR:
                    ::memcpy( iqmData->colors, ( U8* )header + vertexarray->offset, n * sizeof( U8 ) );
                    break;
            }
        }
        ...
        if ( allocateInfluences )
        {
            ...
            for ( i = 0; i < header->num_meshes; i++, surface++ )
            {
                ...
                for ( j = 0; j < ( U32 )surface->num_vertexes; j++ )
                {
                    vtx = surface->first_vertex + j;
                    
                    for ( k = 0; k < ( U32 )surface->num_influences; k++ )
                    {
    ...
    if ( header->num_joints )
    {
        ...
        for ( i = 0; i < header->num_joints; i++, joint++ )
        {
            UTF8* name = reinterpret_cast<UTF8*>( header ) + header->ofs_text + joint->name;
            S32 len = ( S32 )::strlen( name ) + 1;
            ::memcpy( str, name, len );
            str += len;
        }
    ...
    if ( iqmData->num_surfaces && iqmData->num_joints <= glRefConfig.glslMaxAnimatedBones )
    {
        ...
        iqmData->vaoSurfaces = ( srfVaoIQModel_t* )memorySystem->Alloc( sizeof( *iqmData->vaoSurfaces ) * iqmData->numVaoSurfaces, h_low );
        
        vaoSurf = iqmData->vaoSurfaces;
        surf = iqmData->surfaces;
        for ( i = 0; i < ( U32 )iqmData->num_surfaces; i++, vaoSurf++, surf++ )
        {
            S32* tri;
            ...
            for ( j = 0; j < ( U32 )surf->num_vertexes; j++ )
            {
                ...
                // normal
                idRenderSystemVaoLocal::VaoPackNormal( ( S16* )( data + dataOfs ), &iqmData->normals[vtx * 3] );
                dataOfs += sizeof( S16 ) * 4;
                
                // tangent
                idRenderSystemVaoLocal::VaoPackTangent( ( S16* )( data + dataOfs ), &iqmData->tangents[vtx * 4] );
                dataOfs += sizeof( S16 ) * 4;
            ...
            for ( j = 0; j < ( U32 )surf->num_triangles; j++ )
            {
            ...
            vaoSurf->vao = idRenderSystemVaoLocal::CreateVao( va( "staticIQMMesh_VAO '%s'", surf->name ), data, dataSize,
                           ( U8* )indexes, surf->num_triangles * 3 * sizeof( indexes[0] ), VAO_USAGE_STATIC );
...
/*
=============
idRenderSystelModelIQMLocal::CullIQM
=============
*/
S32 idRenderSystelModelIQMLocal::CullIQM( iqmData_t* data, trRefEntity_t* ent )
{
    S32	i;
    F32* oldBounds, *newBounds;
    vec3_t bounds[2];
...
/*
=================
idRenderSystelModelIQMLocal::ComputeIQMFogNum
=================
*/
S32 idRenderSystelModelIQMLocal::ComputeIQMFogNum( iqmData_t* data, trRefEntity_t* ent )
{
    S32 i, j;
    const F32* bounds;
    const F32 defaultBounds[6] = { -8, -8, -8, 8, 8, 8 };
    F32 radius;
    vec3_t diag, center;
    vec3_t localOrigin;
    fog_t* fog;
...
/*
=================
idRenderSystelModelIQMLocal::AddIQMSurfaces

Add all surfaces of this model
=================
*/
void idRenderSystelModelIQMLocal::AddIQMSurfaces( trRefEntity_t* ent )
{
    S32	i, j, cull, fogNum, cubemapIndex;
    iqmData_t* data;
    srfIQModel_t* surface;
    shader_t* shader;
    skin_t* skin;
    ...
    {
        clientMainSystem->RefPrintf( PRINT_DEVELOPER, "idRenderSystelModelIQMLocal::AddIQMSurfaces: no such frame %d to %d for '%s'\n",
    ...
    //
    fogNum = ComputeIQMFogNum( data, ent );
    ...
    for ( i = 0; i < data->num_surfaces; i++ )
    {
        ...
        {
            idRenderSystemMainLocal::AddDrawSurf( ( surfaceType_t* )drawSurf, tr.shadowShader, 0, 0, 0, 0 );
        }
        ...
        if ( r_shadows->integer == 3
                ...
                && shader->sort == SS_OPAQUE )
        {
            idRenderSystemMainLocal::AddDrawSurf( ( surfaceType_t* )drawSurf, tr.projectionShadowShader, 0, 0, 0, 0 );
        }
        
        if ( !personalModel )
        {
            idRenderSystemMainLocal::AddDrawSurf( ( surfaceType_t* )drawSurf, shader, fogNum, 0, 0, cubemapIndex );
        }
...
/*
===============
idRenderSystelModelIQMLocal::ComputePoseMats
===============
*/
void idRenderSystelModelIQMLocal::ComputePoseMats( iqmData_t* data, S32 frame, S32 oldframe, F32 backlerp, F32* poseMats )
{
    S32 i;
    F32* poseMat, lerp;
    const S32* jointParent;
    const F32* invBindMat;
    iqmTransform_t relativeJoints[IQM_MAX_JOINTS], * relativeJoint;
...
/*
===============
idRenderSystelModelIQMLocal::ComputeJointMats
===============
*/
void idRenderSystelModelIQMLocal::ComputeJointMats( iqmData_t* data, S32 frame, S32 oldframe, F32 backlerp, F32* mat )
{
    S32	i;
    F32* mat1;
...
=================
*/
void idRenderSystelModelIQMLocal::IQMSurfaceAnim( surfaceType_t* surface )
{
    ...
    idRenderSystemSurfaceLocal::CheckOverflow( surf->num_vertexes, surf->num_triangles * 3 );
    ...
    if ( data->num_poses > 0 )
    {
                ...
                outXYZ++, outNormal += 4, outTangent += 4, outTexCoord++ )
        {
                ...
                
            {
                ...
                idRenderSystemVaoLocal::VaoPackNormal( outNormal, unpackedNormal );
                ...
                idRenderSystemVaoLocal::VaoPackTangent( outTangent, unpackedTangent );
            }
        }
    }
    else
    {
                ...
                outXYZ++, outNormal += 4, outTangent += 4, outTexCoord++ )
        {
            ...
            idRenderSystemVaoLocal::VaoPackNormal( outNormal, normal );
            idRenderSystemVaoLocal::VaoPackTangent( outTangent, tangent );
        }
    }
...
/*
=================
idRenderSystelModelIQMLocal::IQMSurfaceAnimVao
=================
*/
void idRenderSystelModelIQMLocal::IQMSurfaceAnimVao( srfVaoIQModel_t* surface )
{
    iqmData_t* data = surface->iqmData;
    
    if ( idRenderSystemShadeLocal::ShaderRequiresCPUDeforms( tess.shader ) )
    {
        IQMSurfaceAnim( ( surfaceType_t* )surface->iqmSurface );
        return;
    }
    ...
    //idRenderSystemVaoLocal::CheckVao(surface->vao);
    idRenderSystemShadeLocal::EndSurface();
    idRenderSystemShadeLocal::BeginSurface( tess.shader, tess.fogNum, tess.cubemapIndex );
    
    idRenderSystemVaoLocal::BindVao( surface->vao );
...
/*
===============
idRenderSystelModelIQMLocal::IQMLerpTag
===============
*/
S32 idRenderSystelModelIQMLocal::IQMLerpTag( orientation_t* tag, iqmData_t* data, S32 startFrame, S32 endFrame, F32 frac, StringEntry tagName )
{
    S32	joint;
    F32	jointMats[IQM_MAX_JOINTS * 12];
