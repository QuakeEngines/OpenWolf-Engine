...
void idServerCcmdsSystemLocal::Map_f( void )
{
    ...
    if ( serverGameSystem->GameIsSinglePlayer() )
    {
        ...
        if ( strstr( map, ".sav" ) )
        {
            ...
            {
                Q_snprintf( savedir, sizeof( savedir ), "profiles/%s/save/", cl_profileStr );
            }
            else
            {
            ...
            {
                Q_snprintf( savemap, sizeof( savemap ), "%s%s", savedir, map );
            }
            else
            {
                strcpy( savemap, map );
...
void idServerCcmdsSystemLocal::MapRestart_f( void )
{
    ...
    if ( cvarSystem->VariableIntegerValue( "savegame_loading" ) )
    {
        ...
        {
            Q_snprintf( savemap, sizeof( savemap ), "profiles/%s/save/current.sav", cl_profileStr );
        }
        else
        {
...
void idServerCcmdsSystemLocal::LoadGame_f( void )
{
    ...
    if ( com_sv_running->integer && ( com_frameTime != sv.serverId ) )
    {
        ...
        if ( !Q_stricmp( mapname, sv_mapname->string ) ) // same
        {
            ...
            // 2 means it's a restart, so stop rendering until we are loaded
            cvarSystem->Set( "savegame_loading", "2" );
...
void idServerCcmdsSystemLocal::Status_f( void )
{
    ...
    // Dushan
    cpu = ( U8 )( svs.stats.latched_active + svs.stats.latched_idle );
    
    if ( cpu )
    {
        cpu = ( U8 )( 100 * svs.stats.latched_active / cpu );
    }
    
    avg = ( U8 )( 1000 * svs.stats.latched_active / STATFRAMES );
...
void idServerCcmdsSystemLocal::Demo_Record_f( void )
{
    ...
    if ( cmdSystem->Argc() == 2 )
        sprintf( sv.demoName, "svdemos/%s.svdm_%d", cmdSystem->Argv( 1 ), PROTOCOL_VERSION );
    else
    {
        S32	number;
