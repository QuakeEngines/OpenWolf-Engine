...
{
    UTF8 userinfo[MAX_INFO_STRING], *ip;
    S32	i, clientNum, qport, challenge, startIndex, count, oldInfoLen2 = ( S32 )::strlen( userinfo ), newInfoLen2;
    ...
    {
        ip = ( UTF8* )networkSystem->AdrToString( from );
    }
    ...
    while ( true )
    {
        ...
        newInfoLen2 = ( S32 )::strlen( userinfo );
    ...
    for ( i = 0, cl = svs.clients ; i < sv_maxclients->integer ; i++, cl++ )
    {
        ...
        if ( networkSystem->CompareBaseAdr( from, cl->netchan.remoteAddress ) && ( cl->netchan.qport == qport || from.port == cl->netchan.remoteAddress.port ) )
        {
            ...
            sgame->ClientDisconnect( ( S32 )( newcl - svs.clients ) );
...
void idServerClientSystemLocal::DropClient( client_t* drop, StringEntry reason )
{
    ...
    idServerOACSSystemLocal::ExtendedRecordDropClient( ( S32 )( drop - svs.clients ) );
...
void idServerClientSystemLocal::ClientEnterWorld( client_t* client, usercmd_t* cmd )
{
    ...
    {
        idServerOACSSystemLocal::ExtendedRecordClientConnect( ( S32 )( client - svs.clients ) );
    }
...
void idServerClientSystemLocal::WriteDownloadToClient( client_t* cl, msg_t* msg )
{
    ...
    if ( !cl->download )
    {
        ...
        if ( i == numVersions )
        {
            ...
            Q_snprintf( errorMessage, sizeof( errorMessage ), "Invalid download from update server" );
        ...
        if ( !sv_allowDownload->integer || idPack )
        {
            ...
            if ( !::strstr( cl->downloadName, ".pk3" ) )
            {
                ...
                Q_snprintf( errorMessage, sizeof( errorMessage ), "Cannot download non-pk3 \"%s\"\n", cl->downloadName );
            }
            else if ( ::strstr( cl->downloadName, ".." ) )
            {
                Com_Printf( "Folder hack? %s\n", cl->downloadName );
                Q_snprintf( errorMessage, sizeof( errorMessage ), "Cannot download bad folder \"%s\"\n", cl->downloadName );
            }
            else if ( idPack )
            {
                Com_Printf( "clientDownload: %d : \"%s\" cannot download id pk3 files\n", ( S32 )( cl - svs.clients ), cl->downloadName );
                Q_snprintf( errorMessage, sizeof( errorMessage ), "Cannot autodownload official pk3 file \"%s\"", cl->downloadName );
            }
            else
            {
                ...
                if ( sv_pure->integer )
                {
                    Q_snprintf( errorMessage, sizeof( errorMessage ),
    ...
    while ( cl->downloadCurrentBlock - cl->downloadClientBlock < MAX_DOWNLOAD_WINDOW && cl->downloadSize != cl->downloadCount )
    {
        ...
        {
            cl->downloadBlocks[curindex] = ( U8* )memorySystem->Malloc( MAX_DOWNLOAD_BLKSIZE );
        }
...
void idServerClientSystemLocal::UserinfoChanged( client_t* cl )
{
    ...
    S32 oldInfoLen = ( S32 )::strlen( cl->userinfo );
    S32 newInfoLen;
    
    while ( true )
    {
        Info_RemoveKey( cl->userinfo, "handicap" );
        newInfoLen = ( S32 )::strlen( cl->userinfo );
...
void idServerClientSystemLocal::UpdateUserinfo_f( client_t* cl )
{
    ...
    serverInitSystem->GetConfigstring( CS_PLAYERS + S32( cl - svs.clients ), info, sizeof( info ) );
...
void idServerClientSystemLocal::ExecuteClientCommand( client_t* cl, StringEntry s, bool clientOK, bool premaprestart )
{
    ...
    if ( clientOK )
    {
        ...
        if ( !u->name && sv.state == SS_GAME )
        {
            ...
            {
                if ( cl - svs.clients != serverGameSystem->GameClientNum( ( S32 )( cl - svs.clients ) )->clientNum )
                {
...
void idServerClientSystemLocal::UserMove( client_t* cl, msg_t* msg, bool delta )
{
    ...
    for ( i = 0; i < cmdCount; i++ )
    {
        ...
        serverClientLocal.ClientThink( ( S32 )( cl - svs.clients ), &cmds[i] );
    }
}
