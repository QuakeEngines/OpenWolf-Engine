...
{
    return ( S32 )( ( ( U8* )ent - ( U8* )sv.gentities ) / sv.gentitySize );
}
...
{
    return ( sharedEntity_t* )( ( U8* )sv.gentities + sv.gentitySize * num );
}
...
{
    return ( playerState_t* )( ( U8* )sv.gameClients + sv.gameClientSize * num );
}
...
svEntity_t* idServerGameSystemLocal::SvEntityForGentity( sharedEntity_t* gEnt )
{
    ...
    return &sv.svEntities[NumForGentity( gEnt )];
}
...
{
    return serverGameSystemLocal.GentityNum( ARRAY_INDEX( sv.svEntities, svEnt ) );
}
...
void idServerGameSystemLocal::UpdateSharedConfig( U32 port, StringEntry rconpass )
{
    ...
    networkChainSystem->SendPacket( NS_SERVER, ( S32 )::strlen( message ) + 1, message, to );
}
