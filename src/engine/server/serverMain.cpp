...
void idServerMainSystemLocal::SendServerCommand( client_t* cl, StringEntry fmt, ... )
{
    ...
    va_start( argptr, fmt );
    Q_vsnprintf( reinterpret_cast< UTF8*>( message ), sizeof( message ), fmt, argptr );
    va_end( argptr );
    
    if ( cl != nullptr )
    {
        AddServerCommand( cl, reinterpret_cast<UTF8*>( message ) );
        return;
    }
    ...
    for ( i = 0; i < MAX_MASTER_SERVERS; i++ )
    {
        ...
        if ( sv_master[i]->modified || ( adr[i][0].type == NA_BAD && adr[i][1].type == NA_BAD ) )
        {
            ...
            if ( netenabled & NET_ENABLEV4 )
            {
                ...
                res = networkSystem->StringToAdr( sv_master[i]->string, &adr[i][0], NA_IP );
                ...
                if ( res )
                {
                    Com_Printf( "%s resolved to %s\n", sv_master[i]->string, networkSystem->AdrToStringwPort( adr[i][0] ) );
                }
                else
                {
            ...
            if ( netenabled & NET_ENABLEV6 )
            {
                ...
                res = networkChainSystem->StringToAdr( sv_master[i]->string, &adr[i][1], NA_IP6 );
                ...
                if ( res )
                {
                    Com_Printf( "%s resolved to %s\n", sv_master[i]->string, networkSystem->AdrToStringwPort( adr[i][1] ) );
                }
                else
                {
...
void idServerMainSystemLocal::MasterGameStat( StringEntry data )
{
    ...
    Com_Printf( "%s resolved to %s\n", MASTER_SERVER_NAME, networkSystem->AdrToString( adr ) );
...
bool idServerMainSystemLocal::VerifyChallenge( UTF8* challenge )
{
    ...
    for ( i = 0; i < j; i++ )
    {
        if ( challenge[i] == '\\' || challenge[i] == '/' || challenge[i] == '%' || challenge[i] == ';' || challenge[i] == '"'
                || challenge[i] < 32 || /*// non-ascii */ challenge[i] > 126 )  // non-ascii
        {
            return false;
        }
    }
...
void idServerMainSystemLocal::Status( netadr_t from )
{
    ...
    if ( cvarSystem->VariableValue( "fs_restrict" ) )
    {
        ...
        Q_snprintf( keywords, sizeof( keywords ), "ettest %s", Info_ValueForKey( infostring, "sv_keywords" ) );
    ...
    for ( i = 0; i < sv_maxclients->integer; i++ )
    {
        ...
        if ( cl->state >= CS_CONNECTED )
        {
            ...
            playerLength = ( S32 )::strlen( player );
...
bool idServerMainSystemLocal::CheckDRDoS( netadr_t from )
{
    ...
    {
        // Two minute ban.
        if ( svs.time - ban->time < 120000 && networkSystem->CompareBaseAdr( from, ban->adr ) )
        {
            ban->count++;
...
void idServerMainSystemLocal::Frame( S32 msec )
{
    ...
    frameMsec = ( S32 )( 1000 / sv_fps->integer * com_timescale->value );
    ...
    // run the game simulation in c ut  s
    ...
    if ( com_dedicated->integer )
    {
        ...
        if ( svs.currentFrameIndex == SERVER_PERFORMANCECOUNTER_FRAMES )
        {
            ...
            if ( svs.currentSampleIndex > SERVER_PERFORMANCECOUNTER_SAMPLES )
            {
                ...
                svs.serverLoad = ( averageFrameTime / frameMsec ) * 100;
            }
