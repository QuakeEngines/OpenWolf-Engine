...
void idServerOACSSystemLocal::ExtendedRecordWriteStruct( void )
{
    ...
    file = fileSystem->FOpenFileWrite( sv_oacsTypesFile->string ); // open in write mode
    
    // write the text and free it
    fileSystem->Write( va( "%s\n%s", outheader, out ), ( S32 )::strlen( outheader ) + ( S32 )::strlen( "\n" ) + ( S32 )::strlen( out ), file );
...
void idServerOACSSystemLocal::ExtendedRecordWriteValues( S32 client )
{
    ...
    // Open the data file
    file = fileSystem->FOpenFileAppend( sv_oacsDataFile->string ); // open in append mode
    ...
    if ( fileSystem->FileExists( sv_oacsDataFile->string ) == false || ( fileSystem->IsFileEmpty( sv_oacsDataFile->string ) == true ) )
    {
        ...
        // write the text (with a line return)
        fileSystem->Write( va( "%s\n", outheader ), ( S32 )::strlen( outheader ) + ( S32 )::strlen( "\n" ), file );
        fileSystem->Flush( file ); // update the content of the file
    }
    ...
    for ( i = startclient; i < endclient; i++ )
    {
        ...
        else if ( IsBot( i ) | IsSpectator( i ) ) // avoid saving empty interframes of not yet fully connected players, bots nor spectators
        {
            continue;
        }
                  ...
                  ( ( svs.clients[client].netchan.outgoingSequence - svs.clients[client].deltaMessage ) >= ( PACKET_BACKUP - 3 ) ) )
        ...
        // write the text (with a line return) and free it
        fileSystem->Write( va( "%s\n", out ), ( S32 )::strlen( out ) + ( S32 )::strlen( "\n" ), file );
...
void idServerOACSSystemLocal::ExtendedRecordWritePlayersTable( S32 client )
{
    ...
    file = fileSystem->FOpenFileAppend( sv_oacsPlayersTable->string ); // open in append mode
    
    // If there is no data file or it is empty, we first output the headers (features keys/names)
    if ( fileSystem->FileExists( sv_oacsPlayersTable->string ) == false || ( fileSystem->IsFileEmpty( sv_oacsPlayersTable->string ) == true ) )
    {
        // Output the headers into the text file (only if there's at least one client connected!)
        // write the text (with a line return)
        fileSystem->Write( va( "%s\n", sv_playerstable_keys ), ( S32 )::strlen( sv_playerstable_keys ) + ( S32 )::strlen( "\n" ), file );
...
void idServerOACSSystemLocal::ExtendedRecordInterframeInitValues( S32 client )
{
    ...
    for ( feature = 0; feature < FEATURES_COUNT; feature++ )
    {
        ...
        if ( !IsBot( client ) )
        {
            sv_interframe[feature].value[client] = ExtendedRecordInterframeInitValue( client, feature );
        }
    }
...
void idServerOACSSystemLocal::ExtendedRecordInterframeUpdate( S32 client )
{
    ...
    for ( i = startclient; i < endclient; i++ )
    {
        ...
        if ( sv_interframeModified[i] == true ) // it changed, and the previous one was already committed, so we just have to reset FRAMEREPEAT and the modified flag
        {
...
void idServerOACSSystemLocal::ExtendedRecordInterframeUpdateValues( S32 client )
{
    ...
    // TIMESTAMP
    ExtendedRecordSetFeatureValue( FEATURE_TIMESTAMP, ( F64 )time( nullptr ), client );
    
    // COMMANDTIME_REACTIONTIME
    // must be set before setting the new lastcommandtime
    ExtendedRecordSetFeatureValue( FEATURE_COMMANDTIME_REACTIONTIME, ps->commandTime - sv_interframe[FEATURE_LASTCOMMANDTIME].value[client], client );
    
    // LASTCOMMANDTIME
    ExtendedRecordSetFeatureValue( FEATURE_LASTCOMMANDTIME, ps->commandTime, client );
    
    // ANGLEINAFRAME
    // Compute the total angle in all directions in one frame
    ExtendedRecordSetFeatureValue( FEATURE_ANGLEINAFRAME,
                                   ( abs( ps->viewangles[0] - prev_ps[client].viewangles[0] ) + abs( ps->viewangles[1] - prev_ps[client].viewangles[1] ) +
    ...
    {
        ExtendedRecordSetFeatureValue( FEATURE_MOUSEEVENTTIME_REACTIONTIME, svs.time -
                                       sv_interframe[FEATURE_LASTMOUSEEVENTTIME].value[client], client ); // update the mouse reaction time before the last mouse event time
        ExtendedRecordSetFeatureValue( FEATURE_LASTMOUSEEVENTTIME, svs.time, client );
    }
    
    // MOVEMENTDIR
    ExtendedRecordSetFeatureValue( FEATURE_MOVEMENTDIR, ps->movementDir, client );
    ...
    if ( ( ps->persistant[PERS_HITS] > sv_interframe[FEATURE_HITS].value[client] ) && ( ps->persistant[PERS_SCORE] >
            sv_interframe[FEATURE_SCORE].value[client] ) )
    {
        ExtendedRecordSetFeatureValue( FEATURE_FRAGS, sv_interframe[FEATURE_FRAGS].value[client] + 1, client );
        ExtendedRecordSetFeatureValue( FEATURE_FRAGSINAROW, sv_interframe[FEATURE_FRAGSINAROW].value[client] + 1, client );
    }
    else
    {
    ...
    {
        ExtendedRecordSetFeatureValue( FEATURE_SCOREACC, sv_interframe[FEATURE_SCOREACC].value[client] + ( ps->persistant[PERS_SCORE] -
    ...
    // SCORE
    ExtendedRecordSetFeatureValue( FEATURE_SCORE, ps->persistant[PERS_SCORE], client );
    ...
    {
        ExtendedRecordSetFeatureValue( FEATURE_HITSACC, sv_interframe[FEATURE_HITSACC].value[client] + ( ps->persistant[PERS_HITS] -
    ...
    // HITS
    ExtendedRecordSetFeatureValue( FEATURE_HITS, ps->persistant[PERS_HITS], client );
    ...
    {
        ExtendedRecordSetFeatureValue( FEATURE_DEATHACC, sv_interframe[FEATURE_DEATHACC].value[client] + ( ps->persistant[PERS_KILLED] - sv_interframe[FEATURE_DEATH].value[client] ), client );
    }
    else   // else we reset the accumulator to 0
    {
    ...
    // DEATH
    ExtendedRecordSetFeatureValue( FEATURE_DEATH, ps->persistant[PERS_KILLED], client );
    ...
    {
        ExtendedRecordSetFeatureValue( FEATURE_DAMAGEEVENT_COUNTACC, sv_interframe[FEATURE_DAMAGEEVENT_COUNTACC].value[client] +
                                       ( ps->damageEvent - sv_interframe[FEATURE_DAMAGEEVENT_COUNT].value[client] ), client );
    }
    else
    {
        // else we reset the accumulator to 0
    ...
    // DAMAGEEVENT_COUNT
    ExtendedRecordSetFeatureValue( FEATURE_DAMAGEEVENT_COUNT, ps->damageEvent, client );
    
    // DUCKED
    ExtendedRecordSetFeatureValue( FEATURE_DUCKED, ( ps->pm_flags & PMF_DUCKED ) ? 1 : 0, client );
    
    // MIDAIR
    ExtendedRecordSetFeatureValue( FEATURE_MIDAIR, ps->groundEntityNum == ENTITYNUM_NONE, client );
    
    // WEAPON
    ExtendedRecordSetFeatureValue( FEATURE_WEAPON, ps->weapon, client );
    
    // WEAPONSTATE
    ExtendedRecordSetFeatureValue( FEATURE_WEAPONSTATE, ps->weaponstate, client );
    
    // WEAPONINSTANTHIT
    ExtendedRecordSetFeatureValue( FEATURE_WEAPONINSTANTHIT, IsWeaponInstantHit( ps->weapon ) ? 1 : 0, client );
    
    // HEALTH
    ExtendedRecordSetFeatureValue( FEATURE_HEALTH, ps->stats[STAT_HEALTH], client );
    
    // MAX_HEALTH
    ExtendedRecordSetFeatureValue( FEATURE_MAX_HEALTH, ps->stats[STAT_MAX_HEALTH], client );
    
    // SPEED
    // Total amount of speed in all directions
    ExtendedRecordSetFeatureValue( FEATURE_SPEED, ( abs( ps->velocity[0] ) + abs( ps->velocity[1] ) + abs( ps->velocity[2] ) ), client );
    ...
    // DAMAGE_COUNT
    ExtendedRecordSetFeatureValue( FEATURE_DAMAGE_COUNT, ps->damageCount, client );
    ...
    if ( sv_interframeModified[client] == true )
    {
        ...
        ExtendedRecordSetFeatureValue( FEATURE_REACTIONTIME, svs.time - sv_interframe[FEATURE_SVSTIME].value[client], client );
...
void idServerOACSSystemLocal::ExtendedRecordPlayersTableInit( S32 client )
{
    ...
    // reliable way to get the client's ip adress
    Q_strncpyz( sv_playerstable.ip, networkSystem->AdrToString( cl->netchan.remoteAddress ), MAX_STRING_CHARS );
    ...
    // Set timestamp
    sv_playerstable.timestamp = ( F64 )time( NULL );
...
UTF8* idServerOACSSystemLocal::ExtendedRecordPlayersTableToCSV( UTF8* csv_string, S32 max_string_size, playerstable_t playerstable )
{
    ...
    length += snprintf( csv_string + length, max_string_size, "\"%s\"", playerstable.nickname );
...
void idServerOACSSystemLocal::ExtendedRecordSetFeatureValue( interframeIndex_t feature, F64 value, S32 client )
{
    ...
    if ( ( sv_interframe[feature].value[client] != value ) | isnan( sv_interframe[feature].value[client] ) )
    {
        ...
        if ( sv_interframe[feature].modifier == true && sv_interframeModified[client] != true )
        {
...
bool idServerOACSSystemLocal::IsSpectator( S32 client )
{
    ...
    team = atoi( Info_ValueForKey( sv.configstrings[CS_PLAYERS + client].s, "t" ) ); // Get the team
...
S32 idServerOACSSystemLocal::rand_range( S32 min, S32 max )
{
    ...
    retval = S32( ( ( F64 )rand() / ( F64 )RAND_MAX * ( max - min + 1 ) ) ) + min;
...
void idServerOACSSystemLocal::ExtendedRecordSetCheater_f( void )
{
    ...
    {
        // Set cheater label
        ExtendedRecordSetCheater( atoi( cmdSystem->Argv( 1 ) ), atoi( cmdSystem->Argv( 2 ) ) );
    }
...
void idServerOACSSystemLocal::ExtendedRecordSetCheaterFromClient_f( client_t* cl )
{
    ...
    if ( cmdSystem->Argc() >= 3 )
    {
        if ( atoi( cmdSystem->Argv( 2 ) ) > 0 ) // accept only if it's a cheat type (0 == honest, >= 1 == cheat)
        {
