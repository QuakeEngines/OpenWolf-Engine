...
void idServerSnapshotSystemLocal::SendMessageToClient( msg_t* msg, client_t* client )
{
    ...
    if ( client->netchan.remoteAddress.type == NA_LOOPBACK || ( sv_lanForceRate->integer
            && networkSystem->IsLANAddress( client->netchan.remoteAddress ) ) )
    {
        //client->nextSnapshotTime = svs.time - 1;
        client->nextSnapshotTime = svs.time + ( S32 )( 1000.0f / sv_fps->integer * com_timescale->value );
        return;
    }
    ...
    if ( !*client->downloadName && rateMsec < client->snapshotMsec )
    {
        ...
        rateMsec = ( S32 )( client->snapshotMsec * com_timescale->value );
    ...
    client->nextSnapshotTime = ( S32 )( svs.time + rateMsec * com_timescale->value );
    ...
    if ( client->state != CS_ACTIVE )
    {
        ...
        {
            client->nextSnapshotTime = ( S32 )( svs.time + 1000 * com_timescale->value );
        }
    }
}
...
void idServerSnapshotSystemLocal::CheckClientUserinfoTimer( void )
{
    ...
    for ( i = 0, cl = svs.clients ; i < sv_maxclients->integer ; i++, cl++ )
    {
        ...
        if ( ( sv_floodProtect->integer ) && ( svs.time >= cl->nextReliableUserTime ) && ( cl->state >= CS_ACTIVE ) && ( cl->userinfobuffer[0] != 0 ) )
        {
            ...
            Q_snprintf( bigbuffer, sizeof( bigbuffer ), "userinfo \"%s\"", cl->userinfobuffer );
