...
S32 idServerWallhackSystemLocal::predict_slide_move( sharedEntity_t* ent, F32 frametime, trajectory_t* tr, vec3_t result )
{
    ...
    for ( count = 0; count < NUMBUMPS; count++ )
    {
        ...
        serverWorldSystem->Trace( &trace, origin, ent->r.mins, ent->r.maxs, end, ent->s.number, CONTENTS_SOLID, TT_AABB );
...
void idServerWallhackSystemLocal::predict_move( sharedEntity_t* ent, F32 frametime, trajectory_t* tr, vec3_t result )
{
    ...
    // assume the move fails
    VectorCopy( tr->trBase, result );
    
    // not moving
    if ( zero_vector( tr->trDelta ) )
    {
        return;
    }
    
    // move completed
    if ( predict_slide_move( ent, frametime, tr, result ) )
    {
        return;
    }
    ...
    serverWorldSystem->Trace( &trace, start_o, ent->r.mins, ent->r.maxs, up, ent->s.number, CONTENTS_SOLID, TT_AABB );
    ...
    down[2] -= stepSize;
    serverWorldSystem->Trace( &trace, tr->trBase, ent->r.mins, ent->r.maxs, down, ent->s.number, CONTENTS_SOLID, TT_AABB );
...
===============
*/
void idServerWallhackSystemLocal::calc_viewpoint( playerState_t* ps, vec3_t org, vec3_t vp )
{
    VectorCopy( org, vp );
...
===============
*/
void idServerWallhackSystemLocal::copy_trajectory( trajectory_t* src, trajectory_t* dst )
{
