...
void idServerWorldSystemLocal::ClipMoveToEntities( moveclip_t* clip )
{
    ...
    for ( i = 0; i < num; i++ )
    {
        ...
        collisionModelManager->TransformedBoxTrace( &trace,
                const_cast<F32*>( clip->start ),
                const_cast<F32*>( clip->end ),
                const_cast<F32*>( clip->mins ),
                const_cast<F32*>( clip->maxs ),
                clipHandle,
                clip->contentmask,
                origin, angles,
                clip->collisionType );
...
==================
*/
void idServerWorldSystemLocal::Trace( trace_t* results, const vec3_t start, const vec3_t mins, const vec3_t maxs, const vec3_t end,
                                      S32 passEntityNum, S32 contentmask, traceType_t type )
{
    S32 i;
    moveclip_t clip;
