...
int ScaleBSPMain( int argc, char** argv )
{
    ...
    for ( i = 1; i < argc - 2; i++ )
    {
        if ( !Q_stricmp( argv[i], "-tex" ) )
        {
            texscale = qtrue;
        }
        else if ( !Q_stricmp( argv[i], "-spawn_ref" ) )
        {
    ...
    if ( texscale )
    {
        ...
        old_xyzst = static_cast<float*>( safe_malloc( sizeof( *old_xyzst ) * numBSPDrawVerts * 5 ) );
    ...
    UnparseEntities();
    StripExtension( source );
    DefaultExtension( source, "_s.world" );
