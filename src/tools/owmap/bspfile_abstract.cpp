...
void IncDrawVerts()
{
    ...
    if ( bspDrawVerts == 0 )
    {
        ...
        bspDrawVerts = static_cast<bspDrawVert_t*>( safe_malloc_info( sizeof( bspDrawVert_t ) * numBSPDrawVertsBuffer, "IncDrawVerts" ) );
    ...
    else if ( numBSPDrawVerts > numBSPDrawVertsBuffer )
    {
        ...
        bspDrawVerts = static_cast<bspDrawVert_t*>( realloc( bspDrawVerts, sizeof( bspDrawVert_t ) * numBSPDrawVertsBuffer ) );
...
void SetDrawSurfacesBuffer()
{
    ...
    bspDrawSurfaces = static_cast<bspDrawSurface_t*>( safe_malloc_info( sizeof( bspDrawSurface_t ) * numBSPDrawSurfacesBuffer, "IncDrawSurfaces" ) );
...
epair_t* ParseEPair( void )
{
    ...
    e = static_cast<epair_t*>( safe_malloc( sizeof( epair_t ) ) );
...
qboolean ParseEntity( void )
{
    ...
    AUTOEXPAND_BY_REALLOC( entity_t, entities, numEntities, allocatedEntities, 32 );
...
void UnparseEntities( void )
{
    ...
    /* setup */
    AUTOEXPAND_BY_REALLOC( char, bspEntData, 0, allocatedBSPEntData, 1024 );
    buf = bspEntData;
    end = buf;
    *end = 0;
    ...
    for ( i = 0; i < numBSPEntities && i < numEntities; i++ )
    {
        {
            int sz = end - buf;
            AUTOEXPAND_BY_REALLOC( char, bspEntData, sz + 65536, allocatedBSPEntData, 1024 );
            buf = bspEntData;
            end = buf + sz;
        }
...
void SetKeyValue( entity_t* ent, const char* key, const char* value )
{
    ...
    /* create new epair */
    ep = static_cast<epair_t*>( safe_malloc( sizeof( *ep ) ) );
    ep->next = ent->epairs;
    ent->epairs = ep;
...
qboolean GetVectorForKey( const entity_t* ent, const char* key, vec3_t vec )
{
    ...
    return strlen( k ) > 0 ? qtrue : qfalse;
}
