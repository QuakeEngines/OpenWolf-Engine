...
static void CopyBrushSidesLump( ibspHeader_t* header )
{
    ...
    /* copy */
    in = static_cast<ibspBrushSide_t*>( GetLump( ( bspHeader_t* )header, LUMP_BRUSHSIDES ) );
    for ( i = 0; i < numBSPBrushSides; i++ )
    {
        AUTOEXPAND_BY_REALLOC( bspBrushSide_t, bspBrushSides, i, allocatedBSPBrushSides, 1024 );
        out = &bspBrushSides[i];
...
static void AddBrushSidesLump( FILE* file, ibspHeader_t* header )
{
    ...
    buffer = static_cast<ibspBrushSide_t*>( safe_malloc( size ) );
...
static void CopyDrawSurfacesLump( ibspHeader_t* header )
{
    ...
    /* copy */
    in = static_cast<ibspDrawSurface_t*>( GetLump( ( bspHeader_t* )header, LUMP_SURFACES ) );
    out = bspDrawSurfaces;
...
static void AddDrawSurfacesLump( FILE* file, ibspHeader_t* header )
{
    ...
    buffer = static_cast<ibspDrawSurface_t*>( safe_malloc( size ) );
    memset( buffer, 0, size );
...
static void CopyDrawVertsLump( ibspHeader_t* header )
{
    ...
    /* copy */
    in = static_cast<ibspDrawVert_t*>( GetLump( ( bspHeader_t* )header, LUMP_DRAWVERTS ) );
    out = bspDrawVerts;
...
static void AddDrawVertsLump( FILE* file, ibspHeader_t* header )
{
    ...
    buffer = static_cast<ibspDrawVert_t*>( safe_malloc( size ) );
    memset( buffer, 0, size );
...
static void CopyLightGridLumps( ibspHeader_t* header )
{
    ...
    /* copy */
    in = static_cast<ibspGridPoint_t*>( GetLump( ( bspHeader_t* )header, LUMP_LIGHTGRID ) );
    out = bspGridPoints;
...
static void AddLightGridLumps( FILE* file, ibspHeader_t* header )
{
    ...
    buffer = static_cast<ibspGridPoint_t*>( safe_malloc( numBSPGridPoints * sizeof( *out ) ) );
