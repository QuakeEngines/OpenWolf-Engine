...
static void CopyLightGridLumps( rbspHeader_t* header )
{
    ...
    /* copy */
    inArray = static_cast<unsigned short*>( GetLump( ( bspHeader_t* )header, LUMP_LIGHTARRAY ) );
    in = static_cast<bspGridPoint_t*>( GetLump( ( bspHeader_t* )header, LUMP_LIGHTGRID ) );
    out = bspGridPoints;
...
static void AddLightGridLumps( FILE* file, rbspHeader_t* header )
{
    ...
    gridPoints = static_cast<bspGridPoint_t*>( safe_malloc( maxGridPoints * sizeof( *gridPoints ) ) );
    gridArray = static_cast<unsigned short*>( safe_malloc( numBSPGridPoints * sizeof( *gridArray ) ) );
