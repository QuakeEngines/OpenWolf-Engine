...
int ConvertBSPMain( int argc, char** argv )
{
    ...
    if ( force_map || ( !force_bsp && !Q_stricmp( ext, "map" ) && map_allowed ) )
    {
        if ( !map_allowed )
        {
            Sys_FPrintf( SYS_WRN, "WARNING: the requested conversion should not be done from .map files. Compile a .world first.\n" );
        }
        StripExtension( source );
        DefaultExtension( source, ".map" );
        Sys_Printf( "Loading %s\n", source );
        LoadMapFile( source, qfalse, convertGame == NULL ? qtrue : qfalse );
        sprintf( BSPFilePath, "%s.world", source );
    ...
    if ( convertGame != NULL )
    {
        ...
        /* write bsp */
        StripExtension( source );
        DefaultExtension( source, "_c.world" );
