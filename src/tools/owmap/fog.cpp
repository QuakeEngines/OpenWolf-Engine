...
mesh_t* DrawSurfToMesh( mapDrawSurface_t* ds )
{
    ...
    m = static_cast<mesh_t*>( safe_malloc( sizeof( *m ) ) );
    m->width = ds->patchWidth;
    m->height = ds->patchHeight;
    m->verts = static_cast<bspDrawVert_t*>( safe_malloc( sizeof( m->verts[ 0 ] ) * m->width * m->height ) );
...
void SplitMeshByPlane( mesh_t* in, vec3_t normal, float dist, mesh_t** front, mesh_t** back )
{
    ...
    f = static_cast<mesh_t*>( safe_malloc( sizeof( *f ) ) );
    f->width = split + 2;
    if ( !( f->width & 1 ) )
    {
        f->width++;
    ...
    f->height = in->height;
    f->verts = static_cast<bspDrawVert_t*>( safe_malloc( sizeof( f->verts[0] ) * f->width * f->height ) );
    
    b = static_cast<mesh_t*>( safe_malloc( sizeof( *b ) ) );
    ...
    b->height = in->height;
    b->verts = static_cast<bspDrawVert_t*>( safe_malloc( sizeof( b->verts[0] ) * b->width * b->height ) );
...
qboolean ChopPatchSurfaceByBrush( entity_t* e, mapDrawSurface_t* ds, brush_t* b )
{
    ...
    for ( i = 0; i < numOutside; i++ )
    {
        ...
        newds->verts = static_cast<bspDrawVert_t*>( safe_malloc( newds->numVerts * sizeof( *newds->verts ) ) );
    ...
    if ( numOutside > 0 )
    {
        ...
        free( ds->verts );
        ds->verts = static_cast<bspDrawVert_t*>( safe_malloc( ds->numVerts * sizeof( *ds->verts ) ) );
