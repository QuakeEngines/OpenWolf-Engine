...
void LoadLBM( const char* filename, byte** picture, byte** palette )
{
    ...
    while ( LBM_P < LBMEND_P )
    {
        ...
        switch ( chunktype )
        {
            ...
            case CMAPID:
                cmapbuffer = static_cast<byte*>( safe_malloc( 768 ) );
            ...
            case BODYID:
                ...
                pic_p = picbuffer = static_cast<byte*>( safe_malloc( bmhd.w * bmhd.h ) );
                if ( formtype == PBMID )
                {
                    //
                    // unpack PBM
                   ...
                   int width, int height, byte* palette )
{
    ...
    lbm = lbmptr = static_cast<byte*>( safe_malloc( width * height + 1000 ) );
...
void LoadPCX( const char* filename, byte** pic, byte** palette, int* width, int* height )
{
    ...
    if ( palette )
    {
        *palette = static_cast<byte*>( safe_malloc( 768 ) );
    ...
    out = static_cast<byte*>( safe_malloc( ( pcx->ymax + 1 ) * ( pcx->xmax + 1 ) ) );
    if ( !out )
    {
                   ...
                   int width, int height, byte* palette )
{
    ...
    pcx = static_cast<pcx_t*>( safe_malloc( width * height * 2 + 1000 ) );
...
void LoadBMP( const char* filename, byte** pic, byte** palette, int* width, int* height )
{
    ...
    out = static_cast<byte*>( safe_malloc( bcWidth * bcHeight ) );
    *pic = out;
    pos = bfOffBits;
...
void LoadTGABuffer( const byte* f, const byte* enddata, byte** pic, int* width, int* height )
{
    ...
    *pic = NULL;
    ...
    image_rgba = static_cast<byte*>( safe_malloc( image_width * image_height * 4 ) );
    if ( !image_rgba )
    {
...
void WriteTGA( const char* filename, byte* data, int width, int height )
{
    ...
    buffer = static_cast<byte*>( safe_malloc( width * height * 4 + 18 ) );
    memset( buffer, 0, 18 );
...
void Load32BitImage( const char* name, unsigned** pixels,  int* width, int* height )
{
    ...
    else
    {
        ...
        size = *width * *height;
        pixels32 = static_cast<byte*>( safe_malloc( size * 4 ) );
...
void LoadKTXBufferFirstImage( const byte* buffer, size_t bufSize, byte** pic, int* picWidth, int* picHeight )
{
    ...
    qboolean bigEndian = ( buffer[4] == 4 ) ? qtrue : qfalse;
    ...
    pixels = static_cast<byte*>( safe_malloc( width * height * 4 ) );
    *pic = pixels;
