...
void xml_Select( char* msg, int entitynum, int brushnum, qboolean bError )
{
    ...
    //sprintf( buf, 1024, "Entity %i, Brush %i: %s", entitynum, brushnum, msg );
    ...
    //sprintf( buf, 1024, "%i %i", entitynum, brushnum );
...
{
    Sys_Printf( "Broadcasting is disabled" );
    //address_t address;
    //char sMsg[1024];
    ...
    //if ( brdcst_socket ) {
    //	// send in a header
    //	sprintf( sMsg, "<?xml version=\"1.0\"?><q3map_feedback version=\"1\">" );
    //	NMSG_Clear( &msg );
