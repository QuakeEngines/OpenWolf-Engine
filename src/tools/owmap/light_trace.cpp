...
static int AllocTraceNode( void )
{
    ...
    if ( numTraceNodes >= maxTraceNodes )
    {
        ...
        temp = static_cast<traceNode_t*>( safe_malloc( maxTraceNodes * sizeof( traceNode_t ) ) );
        if ( traceNodes != NULL )
        {
...
static void SubdivideTraceNode_r( int nodeNum, int depth )
{
    ...
    frontNode->items = static_cast<int*>( safe_malloc( frontNode->maxItems * sizeof( *frontNode->items ) ) );
    
    /* setup back node */
    backNode->maxItems = ( node->maxItems >> 1 );
    backNode->items = static_cast<int*>( safe_malloc( backNode->maxItems * sizeof( *backNode->items ) ) );
...
static int TriangulateTraceNode_r( int nodeNum )
{
    ...
    node->items = static_cast<int*>( safe_malloc( node->maxItems * sizeof( tt ) ) );
