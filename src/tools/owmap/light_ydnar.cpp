...
void SmoothNormals( void )
{
    ...
    shadeAngles = static_cast<float*>( safe_malloc( numBSPDrawVerts * sizeof( float ) ) );
...
void IlluminateRawLightmap( int rawLightmapNum )
{
    ...
    else
    {
        ...
        {
            lightLuxels = static_cast<float*>( safe_malloc( llSize ) );
        }
        if ( deluxemap )
        {
            lightDeluxels = static_cast<float*>( safe_malloc( ldSize ) );
        }
        else
        {
            lightDeluxels = NULL;
        ...
        for ( i = 0; i < trace.numLights; i++ )
        {
            ...
            if ( lightSamples > 1 || lightRandomSamples )
            {
                ...
                {
                    lm->superFlags = static_cast<unsigned char*>( safe_malloc( size ) );
                }
            ...
            if ( lm->superLuxels[ lightmapNum ] == NULL )
            {
                /* allocate sampling lightmap storage */
                size = lm->sw * lm->sh * SUPER_LUXEL_SIZE * sizeof( float );
                lm->superLuxels[ lightmapNum ] = static_cast<float*>( safe_malloc( size ) );
                memset( lm->superLuxels[ lightmapNum ], 0, size );
            }
...
void IlluminateVertexes( int num )
{
    ...
    if ( lm == NULL || g_forceVertex )
    {
        /* setup trace */
        trace.testOcclusion = ( g_forceVertex && lm != NULL ) ? qfalse : !noTrace ? qtrue : qfalse;
...
void SetupBrushesFlags( unsigned int mask_any, unsigned int test_any, unsigned int mask_all, unsigned int test_all )
{
    ...
    {
        opaqueBrushes = static_cast<byte*>( safe_malloc( numBSPBrushes / 8 + 1 ) );
    }
...
void CreateTraceLightsForBounds( vec3_t mins, vec3_t maxs, vec3_t normal, int numClusters, int* clusters, int flags, trace_t* trace )
{
    ...
    trace->lights = static_cast<light_t**>( safe_malloc( sizeof( light_t* ) * ( numLights + 1 ) ) );
    trace->numLights = 0;
