...
void WriteTGA24( char* filename, byte* data, int width, int height, qboolean flip )
{
    ...
    buffer = static_cast<byte*>( safe_malloc( width * height * 3 + 18 ) );
    memset( buffer, 0, 18 );
    buffer[ 2 ] = 2;
    buffer[ 12 ] = width & 255;
    buffer[ 13 ] = width >> 8;
...
void FinishRawLightmap( rawLightmap_t* lm )
{
    ...
    lm->lightClusters = static_cast<int*>( safe_malloc( lm->numLightClusters * sizeof( *lm->lightClusters ) ) );
    c = 0;
    for ( i = 0; i < lm->numLightSurfaces; i++ )
    {
        /* get surface info */
    ...
    {
        lm->bspLuxels[ 0 ] = static_cast<float*>( safe_malloc( size ) );
    }
    ...
    if ( bounce )
    {
        ...
        {
            lm->radLuxels[ 0 ] = static_cast<float*>( safe_malloc( size ) );
        }
    ...
    {
        lm->superLuxels[ 0 ] = static_cast<float*>( safe_malloc( size ) );
    }
    ...
    {
        lm->superOrigins = static_cast<float*>( safe_malloc( size ) );
    }
    ...
    {
        lm->superNormals = static_cast<float*>( safe_malloc( size ) );
    }
    ...
    {
        lm->superFloodLight = static_cast<float*>( safe_malloc( size ) );
    }
    ...
    {
        lm->superClusters = static_cast<int*>( safe_malloc( size ) );
    }
    size = lm->sw * lm->sh;
    sc = lm->superClusters;
    for ( i = 0; i < size; i++ )
    ...
    if ( deluxemap )
    {
        ...
        {
            lm->superDeluxels = static_cast<float*>( safe_malloc( size ) );
        }
        ...
        {
            lm->bspDeluxels = static_cast<float*>( safe_malloc( size ) );
        }
...
qboolean AddSurfaceToRawLightmap( int num, rawLightmap_t* lm )
{
    ...
    if ( VectorLength( ds->lightmapVecs[ 2 ] ) || 1 )    /* ydnar: can't remember what exactly i was thinking here... */
    {
        ...
        lm->vecs = static_cast<vec3_t*>( safe_malloc( 3 * sizeof( vec3_t ) ) );
...
void SetupSurfaceLightmaps( void )
{
    ...
    surfaceClusters = static_cast<int*>( safe_malloc( maxSurfaceClusters * sizeof( *surfaceClusters ) ) );
    memset( surfaceClusters, 0, maxSurfaceClusters * sizeof( *surfaceClusters ) );
    
    /* allocate a list for per-surface info */
    surfaceInfos = static_cast<surfaceInfo_t*>( safe_malloc( numBSPDrawSurfaces * sizeof( *surfaceInfos ) ) );
    ...
    sortSurfaces = static_cast<int*>( safe_malloc( numBSPDrawSurfaces * sizeof( int ) ) );
    ...
    for ( i = 0; i < numBSPModels; i++ )
    {
        ...
        for ( j = 0; j < model->numBSPSurfaces; j++ )
        {
            ...
            {
                /* make a plane */
                info->plane = static_cast<float*>( safe_malloc( 4 * sizeof( float ) ) );
    ...
    numLightSurfaces = 0;
    lightSurfaces = static_cast<int*>( safe_malloc( numSurfsLightmapped * sizeof( int ) ) );
    memset( lightSurfaces, 0, numSurfsLightmapped * sizeof( int ) );
    
    /* allocate a list of raw lightmaps */
    numRawSuperLuxels = 0;
    numRawLightmaps = 0;
    rawLightmaps = static_cast<rawLightmap_t*>( safe_malloc( numSurfsLightmapped * sizeof( *rawLightmaps ) ) );
    ...
    {
        vertexLuxels[ k ] = static_cast<float*>( safe_malloc( numBSPDrawVerts * VERTEX_LUXEL_SIZE * sizeof( float ) ) );
...
static void SetupOutLightmap( rawLightmap_t* lm, outLightmap_t* olm )
{
    ...
    /* allocate buffers */
    olm->lightBits = static_cast<byte*>( safe_malloc( ( olm->customWidth * olm->customHeight / 8 ) + 8 ) );
    memset( olm->lightBits, 0, ( olm->customWidth * olm->customHeight / 8 ) + 8 );
    
    olm->bspLightBytes = static_cast<byte*>( safe_malloc( olm->customWidth * olm->customHeight * 3 ) );
    ...
    if ( deluxemap )
    {
        olm->bspDirBytes = static_cast<byte*>( safe_malloc( olm->customWidth * olm->customHeight * 3 ) );
...
static void FindOutLightmaps( rawLightmap_t* lm )
{
    ...
    for ( lightmapNum = 0; lightmapNum < MAX_LIGHTMAPS; lightmapNum++ )
    {
        ...
        if ( ok == qfalse )
        {
            ...
            olm = static_cast<outLightmap_t*>( safe_malloc( numOutLightmaps * sizeof( outLightmap_t ) ) );
...
void FillOutLightmap( outLightmap_t* olm )
{
    ...
    lightBitsNew = static_cast<byte*>( safe_malloc( ( olm->customWidth * olm->customHeight + 8 ) / 8 ) );
    lightBytesNew = static_cast<byte*>( safe_malloc( olm->customWidth * olm->customHeight * 3 ) );
    if ( deluxemap )
    {
        dirBytesNew = static_cast<byte*>( safe_malloc( olm->customWidth * olm->customHeight * 3 ) );
    }
...
void StoreSurfaceLightmaps( void )
{
    ...
    for ( i = 0; i < numRawLightmaps; i++ )
    {
        ...
        for ( lightmapNum = 0; lightmapNum < MAX_LIGHTMAPS; lightmapNum++ )
        {
            ...
            {
                size = lm->w * lm->h * BSP_LUXEL_SIZE * sizeof( float );
                lm->bspLuxels[ lightmapNum ] = static_cast<float*>( safe_malloc( size ) );
            ...
            if ( bounce )
            {
                ...
                {
                    lm->radLuxels[ lightmapNum ] = static_cast<float*>( safe_malloc( size ) );
                }
    ...
    {
        sortLightmaps = static_cast<int*>( safe_malloc( numRawLightmaps * sizeof( int ) ) );
    }
    ...
    else
    {
        ...
        bspLightBytes = static_cast<byte*>( safe_malloc( numBSPLightBytes ) );
    ...
    for ( i = 0; i < numBSPDrawSurfaces; i++ )
    {
        ...
        if ( olm != NULL && lm != NULL && lm->styles[ 1 ] != LS_NONE ) //% info->si->styleMarker > 0 )
        {
            qboolean dfEqual;
            ...
            /* setup */
            sprintf( styleStages, "\n\t// OWMap custom lightstyle stage(s)\n" );
            dv = &bspDrawVerts[ ds->firstVert ];
