...
int CreateNewFloatPlane( vec3_t normal, vec_t dist )
{
    ...
    // create a new plane
    AUTOEXPAND_BY_REALLOC( plane_t, mapplanes, nummapplanes + 1, allocatedmapplanes, 1024 );
...
void LoadEntityIndexMap( entity_t* e )
{
    ...
    if ( !Q_stricmp( ext, "tga" ) )
    {
        ...
        /* convert to bytes */
        size = w * h;
        pixels = static_cast<byte*>( safe_malloc( size ) );
        for ( i = 0; i < size; i++ )
        {
    ...
    im = static_cast<indexMap_t*>( safe_malloc( sizeof( *im ) ) );
...
static void injectMapCoords( vec3_t mins, vec3_t maxs )
{
    ...
    char buf[256];
    sprintf( buf, "%0.f %0.f", top[0], top[1] );
    SetKeyValue( &entities[0], "mapcoordsmins", buf );
    sprintf( buf, "%0.f %0.f", bot[0], bot[1] );
