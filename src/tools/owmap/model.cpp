...
void InsertModel( const char* name, int skin, int frame, m4x4_t transform, remap_t* remap, shaderInfo_t* celShader, int eNum, int castShadows, int recvShadows, int spawnFlags, float lightmapScale, int lightmapSampleSize, float shadeAngle )
{
    ...
    if ( skinfilesize >= 0 )
    {
        ...
        for ( skinfileptr = skinfilecontent; *skinfileptr; skinfileptr = skinfilenextptr )
        {
            ...
            /* create new item */
            sf2 = sf;
            sf = static_cast<skinfile_t*>( safe_malloc( sizeof( *sf ) ) );
            sf->next = sf2;
    ...
    for ( s = 0; s < numSurfaces; s++ )
    {
        ...
        ds->verts = static_cast<bspDrawVert_t*>( safe_malloc( ds->numVerts * sizeof( ds->verts[ 0 ] ) ) );
        memset( ds->verts, 0, ds->numVerts * sizeof( ds->verts[ 0 ] ) );
        
        ds->numIndexes = PicoGetSurfaceNumIndexes( surface );
        ds->indexes = static_cast<int*>( safe_malloc( ds->numIndexes * sizeof( ds->indexes[ 0 ] ) ) );
        ...
        if ( si->clipModel || ( spawnFlags & 2 ) )   /* 2nd bit */
        {
            ...
            {
                /* overflow hack */
                AUTOEXPAND_BY_REALLOC( plane_t, mapplanes, ( nummapplanes + 64 ) << 1, allocatedmapplanes, 1024 );
    ...
    for ( num = 1; num < numEntities; num++ )
    {
        ...
        for ( ep = e2->epairs; ep != NULL; ep = ep->next )
        {
                    ...
                    !Q_strncasecmp( ep->key, "_remap", 6 ) )
            {
                ...
                remap2 = remap;
                remap = static_cast<remap_t*>( safe_malloc( sizeof( *remap ) ) );
                remap->next = remap2;
