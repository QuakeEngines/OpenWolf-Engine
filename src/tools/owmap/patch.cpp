...
void ParsePatch( qboolean onlyLights )
{
    ...
    m.width = info[0];
    m.height = info[1];
    m.verts = verts = static_cast<bspDrawVert_t*>( safe_malloc( m.width * m.height * sizeof( m.verts[0] ) ) );
    ...
    /* allocate patch mesh */
    pm = static_cast<parseMesh_t*>( safe_malloc( sizeof( *pm ) ) );
...
void PatchMapDrawSurfs( entity_t* e )
{
    ...
    bordering = static_cast<byte*>( safe_malloc( patchCount * patchCount ) );
