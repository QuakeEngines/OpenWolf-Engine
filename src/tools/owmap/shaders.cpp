...
void WriteMapShaderFile( void )
{
    ...
    /* print header */
    fprintf( file,
             "// Custom shader file for %s.world\n"
             "// Generated by OWMap\n"
...
shaderInfo_t* CustomShader( shaderInfo_t* si, char* find, char* replace )
{
    ...
    csi->shaderText = static_cast<char*>( safe_malloc( strlen( shaderText ) + 1 ) );
...
static shaderInfo_t* AllocShaderInfo( void )
{
    ...
    if ( shaderInfo == NULL )
    {
        shaderInfo = static_cast<shaderInfo_t*>( safe_malloc( sizeof( shaderInfo_t ) * MAX_SHADER_INFO ) );
        numShaderInfo = 0;
    }
...
void ParseMaterial( shaderInfo_t* si, const char* token )
{
    ...
    if ( token[0] == 0 )
    {
        if ( Q_stricmp( token, "sky" ) && Q_stricmp( token, "sun" ) )
        {
...
static void ParseShaderFile( const char* filename )
{
    ...
    while ( 1 )
    {
        ...
        {
            strcat( shaderText, "\n" );
            si->shaderText = static_cast<char*>( safe_malloc( strlen( shaderText ) + 1 ) );
        ...
        while ( 1 )
        {
            ...
            else if ( !Q_stricmp( token, "damageShader" ) )
            {
                ...
                if ( token[ 0 ] != '\0' )
                {
                    si->damageShader = static_cast<char*>( safe_malloc( strlen( token ) + 1 ) );
            ...
            else if ( !Q_stricmp( token, "sun" ) /* sof2 */ || !Q_stricmp( token, "q3map_sun" ) || !Q_stricmp( token, "q3map_sunExt" ) )
            {
                ...
                /* allocate sun */
                sun = static_cast<sun_t*>( safe_malloc( sizeof( *sun ) ) );
            ...
            else if ( !Q_strncasecmp( token, "q3map_", 6 ) )
            {
                ...
                else if ( !Q_stricmp( token, "q3map_surfacemodel" ) )
                {
                    ...
                    model = static_cast<surfaceModel_t*>( safe_malloc( sizeof( *model ) ) );
                ...
                else if ( !Q_stricmp( token, "q3map_foliage" ) )
                {
                    ...
                    foliage = static_cast<foliage_t*>( safe_malloc( sizeof( *foliage ) ) );
                ...
                else if ( !Q_stricmp( token, "q3map_flare" ) || !Q_stricmp( token, "q3map_flareShader" ) )
                {
                    ...
                    if ( token[ 0 ] != '\0' )
                    {
                        si->flareShader = static_cast<char*>( safe_malloc( strlen( token ) + 1 ) );
                ...
                else if ( !Q_stricmp( token, "q3map_backShader" ) )
                {
                    ...
                    if ( token[ 0 ] != '\0' )
                    {
                        si->backShader = static_cast<char*>( safe_malloc( strlen( token ) + 1 ) );
                ...
                else if ( !Q_stricmp( token, "q3map_cloneShader" ) )
                {
                    ...
                    if ( token[ 0 ] != '\0' )
                    {
                        si->cloneShader = static_cast<char*>( safe_malloc( strlen( token ) + 1 ) );
                ...
                else if ( !Q_stricmp( token, "q3map_remapShader" ) )
                {
                    ...
                    if ( token[ 0 ] != '\0' )
                    {
                        si->remapShader = static_cast<char*>( safe_malloc( strlen( token ) + 1 ) );
                ...
                else if ( !Q_stricmp( token, "q3map_deprecateShader" ) )
                {
                    ...
                    if ( token[ 0 ] != '\0' )
                    {
                        ...
                        si->deprecateShader = static_cast<char*>( safe_malloc( strlen( token ) + 1 ) );
                          ...
                          !Q_stricmp( token, "q3map_alphaGen" ) || !Q_stricmp( token, "q3map_alphaMod" ) )
                {
                    ...
                    cm = static_cast<colorMod_t*>( safe_malloc( sizeof( *cm ) ) );
                    ...
                    {
                        cm->type = static_cast<colorModType_t>( CM_COLOR_DOT_PRODUCT + alpha );
                        Parse1DMatrixAppend( shaderText, 3, cm->data );
                    }
                    ...
                    {
                        cm->type = static_cast<colorModType_t>( CM_COLOR_DOT_PRODUCT_SCALE + alpha );
                    ...
                    {
                        cm->type = static_cast<colorModType_t>( CM_COLOR_DOT_PRODUCT_2 + alpha );
                        Parse1DMatrixAppend( shaderText, 3, cm->data );
                    }
                    ...
                    {
                        cm->type = static_cast<colorModType_t>( CM_COLOR_DOT_PRODUCT_2_SCALE + alpha );
                        Parse1DMatrixAppend( shaderText, 5, cm->data );
                    }
...
static void ParseCustomInfoParms( void )
{
    ...
    while ( 1 )
    {
        ...
        custSurfaceParms[ numCustSurfaceParms ].name = static_cast<char*>( safe_malloc( MAX_OS_PATH ) );
