...
mapDrawSurface_t* CloneSurface( mapDrawSurface_t* src, shaderInfo_t* si )
{
    ...
    ds->indexes = static_cast<int*>( safe_malloc( ds->numIndexes * sizeof( *ds->indexes ) ) );
...
void ClipSidesIntoTree( entity_t* e, tree_t* tree )
{
    ...
    for ( b = e->brushes; b; b = b->next )
    {
        ...
        for ( i = 0; i < b->numsides; i++ )
        {
            ...
            w = ReverseWinding( w );
            newSide = static_cast<side_t*>( safe_malloc( sizeof( *side ) ) );
            *newSide = *side;
...
static void OptimizeTriangleSurface( mapDrawSurface_t* ds )
{
    ...
    indexes = static_cast<int*>( safe_malloc( ds->numIndexes * sizeof( *indexes ) ) );
