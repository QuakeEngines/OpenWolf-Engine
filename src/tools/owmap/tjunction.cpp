...
void InsertPointOnEdge( vec3_t v, edgeLine_t* e )
{
    ...
    p = static_cast<edgePoint_t*>( safe_malloc( sizeof( edgePoint_t ) ) );
    p->intercept = d;
    VectorCopy( v, p->xyz );
...
int AddEdge( vec3_t v1, vec3_t v2, qboolean createNonAxial )
{
    ...
    if ( !createNonAxial )
    {
        ...
        {
            AUTOEXPAND_BY_REALLOC( originalEdge_t, originalEdges, numOriginalEdges, allocatedOriginalEdges, 1024 );
    ...
    // create a new edge
    AUTOEXPAND_BY_REALLOC( edgeLine_t, edgeLines, numEdgeLines, allocatedEdgeLines, 1024 );
    
    e = &edgeLines[ numEdgeLines ];
    numEdgeLines++;
    
    e->chain = static_cast<edgePoint_t*>( safe_malloc( sizeof( edgePoint_t ) ) );
...
void FixSurfaceJunctions( mapDrawSurface_t* ds )
{
    ...
    if ( i == 0 )
    {
        ...
        ds->verts = static_cast<bspDrawVert_t*>( safe_malloc( numVerts * sizeof( *ds->verts ) ) );
...
qboolean FixBrokenSurface( mapDrawSurface_t* ds )
{
    ...
    return ds->numVerts >= 3 ? qtrue : qfalse;
}
