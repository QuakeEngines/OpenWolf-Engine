...
fixedWinding_t* NewFixedWinding( int points )
{
    ...
    w = static_cast<fixedWinding_t*>( safe_malloc( size ) );
    memset( w, 0, size );
...
void LoadPortals( char* name )
{
    ...
    portals = static_cast<vportal_t*>( safe_malloc( 2 * numportals * sizeof( vportal_t ) ) );
    memset( portals, 0, 2 * numportals * sizeof( vportal_t ) );
    
    leafs = static_cast<leaf_t*>( safe_malloc( portalclusters * sizeof( leaf_t ) ) );
    ...
    for ( i = 0, p = portals ; i < numportals ; i++ )
    {
        ...
        p->num = i + 1;
        p->hint = ( ( flags & 1 ) != 0 ) ? qtrue : qfalse;
        p->sky = ( ( flags & 2 ) != 0 ) ? qtrue : qfalse;
        p->winding = w;
    ...
    faces = static_cast<vportal_t*>( safe_malloc( 2 * numfaces * sizeof( vportal_t ) ) );
    memset( faces, 0, 2 * numfaces * sizeof( vportal_t ) );
    
    faceleafs = static_cast<leaf_t*>( safe_malloc( portalclusters * sizeof( leaf_t ) ) );
...
int VisMain( int argc, char** argv )
{
    ...
    for ( i = 1 ; i < ( argc - 1 ) ; i++ )
    {
        ...
        else if ( !Q_stricmp( argv[i], "-saveprt" ) )
        {
            saveprt = qtrue;
