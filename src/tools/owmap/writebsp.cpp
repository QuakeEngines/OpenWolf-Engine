...
int EmitShader( const char* shader, int* contentFlags, int* surfaceFlags )
{
    ...
    /* emit a new shader */
    AUTOEXPAND_BY_REALLOC_BSP( bspShader_t, Shaders, 1024 );
...
void EmitPlanes( void )
{
    ...
    {
        AUTOEXPAND_BY_REALLOC_BSP( bspPlane_t, Planes, 1024 );
...
void EmitLeaf( node_t* node )
{
    ...
    for ( b = node->brushlist; b; b = b->next )
    {
        ...
        AUTOEXPAND_BY_REALLOC_BSP( int, LeafBrushes, 1024 );
    ...
    {
        AUTOEXPAND_BY_REALLOC_BSP( int, LeafSurfaces, 1024 );
...
int EmitDrawNode_r( node_t* node )
{
    ...
    /* emit a node */
    AUTOEXPAND_BY_REALLOC_BSP( bspNode_t, Nodes, 1024 );
    n0 = numBSPNodes;
    n = &bspNodes[ n0 ];
    numBSPNodes++;
...
void BeginBSPFile( void )
{
    ...
    numBSPDrawIndexes = 6;
    AUTOEXPAND_BY_REALLOC_BSP( int, DrawIndexes, 1024 );
    bspDrawIndexes[ 0 ] = 0;
    bspDrawIndexes[ 1 ] = 1;
    bspDrawIndexes[ 2 ] = 2;
    bspDrawIndexes[ 3 ] = 0;
...
void EmitBrushes( brush_t* brushes, int* firstBrush, int* numBrushes )
{
    ...
    {
        /* check limits */
        AUTOEXPAND_BY_REALLOC_BSP( bspBrush_t, Brushes, 1024 );
        ...
        for ( j = 0; j < b->numsides; j++ )
        {
            ...
            /* check count */
            AUTOEXPAND_BY_REALLOC_BSP( bspBrushSide_t, BrushSides, 1024 );
...
void BeginModel( void )
{
    ...
    /* test limits */
    AUTOEXPAND_BY_REALLOC_BSP( bspModel_t, Models, 256 );
...
void EndModel( entity_t* e, node_t* headnode )
{
    ...
    /* vortex: check mins/maxs */
    for ( i = 0; i < 3; i++ )
    {
        if ( mod->mins[i] > mod->maxs[i] )
        {
            mod->mins[i] = 0;
            mod->maxs[i] = 0;
        }
